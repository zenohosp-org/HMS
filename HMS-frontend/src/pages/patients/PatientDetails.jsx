...
const TYPE_META = {
  ...
  DIAGNOSIS: {
    ...
    darkColor: "dark:bg-slate-500/10 dark:text-slate-500 dark:border-slate-900 dark:border-white/20",
...
function PatientDetails() {
  ...
  if (!patient) return <div className="flex flex-col items-center justify-center h-64 gap-3"><AlertCircle className="w-10 h-10 text-slate-500 dark:text-[#777777]" /><p className="text-slate-500 dark:text-[#666666]">Patient not found.</p><button className="btn-secondary text-sm" onClick={() => navigate(-1)}>← Go Back</button></div>;
  ...
  ><Edit2 className="w-4 h-4 text-slate-600 dark:text-[#cccccc]" />
    ...
    ><ReceiptText className="w-4 h-4 text-slate-600 dark:text-[#cccccc]" />
      Print Invoice
        ...
        /* Avatar + Name block */
                        ...
                        flex items-center justify-center text-2xl font-bold text-slate-700 dark:text-white">{patient.firstName[0]}{patient.lastName?.[0] ?? ""}</div><h2 className="text-lg font-bold text-slate-900 dark:text-white leading-tight">{patient.firstName} {patient.lastName}</h2>{age !== null && <p className="text-sm text-slate-500 dark:text-[#888888] mt-0.5">{age} years · {patient.gender}</p>}<div className="flex items-center justify-center gap-2 mt-3"><span className="inline-flex items-center gap-1.5 px-3 py-1 rounded-full text-xs font-semibold bg-emerald-50 dark:bg-emerald-500/10 text-emerald-700 dark:text-emerald-400 border border-emerald-200 dark:border-emerald-500/20">
          Active
        </span>{blood && <span className={`px-2.5 py-1 rounded-full text-xs font-semibold border
                                bg-red-50 text-red-700 border-red-200 ${bloodDarkClass}`}><Droplets className="inline w-3 h-3 mr-0.5" />{blood}</span>}</div><p className="text-xs text-slate-600 dark:text-[#888888] mt-3">{patient.mrn}</p></div>{
        /* Sections */
    ...
    >{t === "records" ? `Records ${!recordsLoading ? `(${records.length})` : ""}` : t === "appointments" ? `Appointments ${!appointmentsLoading ? `(${appointments.length})` : ""}` : t === "radiology" ? `Radiology ${!radiologyLoading ? `(${radiologyOrders.length})` : ""}` : t === "billing" ? `Billing ${!invoicesLoading ? `(${invoices.length})` : ""}` : "Overview"}</button>)}</div>{
        ...
        ><Plus className="w-3.5 h-3.5" />{showAddRecord ? "Cancel" : "Add Record"}</button></div>{
          ...
          Dr. {appt.doctorName}</p>{appt.doctorSpecialization && <p className="text-xs text-slate-500 dark:text-slate-400">{appt.doctorSpecialization}</p>}</div></div><div className="pt-3 border-t border-slate-100 dark:border-[#222222] grid grid-cols-2 gap-3"><div><p className="text-[10px] uppercase font-bold text-slate-600 dark:text-[#666666]">Type</p><p className="text-xs font-semibold text-slate-700 dark:text-slate-500 mt-0.5">{appt.type}</p></div>{appt.tokenNumber && <div><p className="text-[10px] uppercase font-bold text-slate-600 dark:text-[#666666]">Token No</p><p className="text-xs font-semibold text-slate-700 dark:text-slate-500 mt-0.5">
            ...
            </p><p className="text-xs text-slate-700 dark:text-slate-500 leading-relaxed italic">
                "{appt.chiefComplaint}"
        ...
        >
          + New Invoice
        </button></div>{invoicesLoading ? <div className="flex justify-center py-10"><Loader2 className="w-5 h-5 animate-spin text-slate-500" /></div> : invoices.length === 0 ? <div className="py-12 text-center"><p className="text-sm text-slate-600 dark:text-[#999999]">No invoices yet</p><button onClick={() => navigate(`/billing?patientId=${id}`)} className="mt-3 text-sm font-semibold text-slate-900 dark:text-white dark:text-slate-500 hover:underline">
