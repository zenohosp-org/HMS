...
const TYPE_META = {
  ...
  DIAGNOSIS: {
    label: "Diagnosis",
    color: "bg-slate-100 dark:bg-[#1e1e1e] text-slate-900 dark:text-white border-emerald-200",
    darkColor: "dark:bg-slate-500/10 dark:text-slate-300 dark:border-slate-900 dark:border-white/20",
  ...
  return <div className="flex gap-4 group">{
    ...
    /* Card */
                    ...
                    rounded-lg p-5 hover:border-slate-300 dark:hover:border-[#2a2a2a] transition-colors"><div className="flex items-start justify-between gap-3 mb-3"><span className={`text-[11px] font-semibold uppercase tracking-wide px-3 py-1 rounded-full border
                            ${meta.color} ${meta.darkColor}`}>{meta.label}</span><span className="text-xs text-slate-600 dark:text-[#555555] shrink-0 flex items-center gap-1.5"><Clock className="w-3.5 h-3.5" />{formatDateTime(record.createdAt)}</span></div>{record.description && <p className="text-base font-semibold text-slate-800 dark:text-[#e0e0e0] leading-snug whitespace-pre-wrap mt-2">{record.description}</p>}{record.nextVisitDate && <div className="flex items-center gap-1.5 mt-4 text-xs font-semibold text-slate-900 dark:text-white dark:text-slate-300"><Calendar className="w-4 h-4" /><span>Next visit: {formatDateTime(record.nextVisitDate)}</span></div>}<div className="flex items-center gap-1.5 mt-3 text-xs text-slate-600 dark:text-[#555555]"><Stethoscope className="w-4 h-4 text-slate-500" /><span>{record.createdBy.firstName} {record.createdBy.lastName}<span className="text-slate-300 dark:text-[#444444]"> · {record.createdBy.role}</span></span></div></div></div></div>;
}
...
function PatientDetails() {
  ...
  ><Edit2 className="w-4 h-4 text-slate-600 dark:text-[#cccccc]" />
                                    Edit Patient
                                </button><button
    onClick={() => {
      setTab("records");
    ...
    className="w-full flex items-center gap-2.5 px-3 py-2.5 text-sm
                                        text-slate-700 dark:text-[#cccccc] hover:text-slate-900 hover:bg-slate-50 dark:hover:text-white dark:hover:bg-[#222222] transition-colors text-left"
  ><ClipboardList className="w-4 h-4 text-slate-600 dark:text-[#cccccc]" />
                                    Add Record
                                </button><button
    onClick={() => {
    ...
    className="w-full flex items-center gap-2.5 px-3 py-2.5 text-sm
                                        text-slate-700 dark:text-[#cccccc] hover:text-slate-900 hover:bg-slate-50 dark:hover:text-white dark:hover:bg-[#222222] transition-colors text-left"
  ><ReceiptText className="w-4 h-4 text-slate-600 dark:text-[#cccccc]" />
                                    Print Invoice
    ...
    /* Avatar + Name block */
                        ...
                        flex items-center justify-center text-2xl font-bold text-slate-700 dark:text-white">{patient.firstName[0]}{patient.lastName?.[0] ?? ""}</div><h2 className="text-lg font-bold text-slate-900 dark:text-white leading-tight">{patient.firstName} {patient.lastName}</h2>{age !== null && <p className="text-sm text-slate-500 dark:text-[#888888] mt-0.5">{age} years · {patient.gender}</p>}<div className="flex items-center justify-center gap-2 mt-3"><span className="px-2.5 py-1 rounded-full text-xs font-semibold bg-slate-500/10 text-slate-300 dark:text-white border border-slate-900 dark:border-white/20">
                            Active
                                ...
                                bg-red-50 text-red-700 border-red-200 ${bloodDarkClass}`}><Droplets className="inline w-3 h-3 mr-0.5" />{blood}</span>}</div><p className="text-xs text-slate-600 dark:text-[#444444] mt-3">{patient.mrn}</p></div>{
    /* Sections */
    ...
    /* Room Allocation */
  }{allocatedRoom && <div><SectionHeader icon={<Bed className="w-4 h-4" />} title="Allocation" /><div className="bg-slate-100 dark:bg-[#1e1e1e] dark:bg-slate-500/10 border border-emerald-200 dark:border-slate-900 dark:border-white/20 rounded-lg p-3"><p className="text-xs font-semibold text-emerald-800 dark:text-slate-300 mb-1">Current Room</p><div className="flex items-center gap-2"><p className="text-sm font-bold text-emerald-900 dark:text-emerald-300">{allocatedRoom.roomNumber}</p><span className="text-[10px] uppercase font-bold px-1.5 py-0.5 rounded bg-white dark:bg-[#111111] text-slate-900 dark:text-white dark:text-slate-300 border border-emerald-200 dark:border-slate-900 dark:border-white/30">{allocatedRoom.roomType}</span></div>{allocatedRoom.approxDischargeTime && <p className="text-xs text-slate-900 dark:text-white dark:text-slate-900 dark:text-white mt-2 flex items-center gap-1.5"><CalendarClock className="w-3.5 h-3.5" />
  ...
  >{t === "records" ? `Records ${!recordsLoading ? `(${records.length})` : ""}` : t === "appointments" ? `Appointments ${!appointmentsLoading ? `(${appointments.length})` : ""}` : t === "radiology" ? `Radiology ${!radiologyLoading ? `(${radiologyOrders.length})` : ""}` : t === "billing" ? `Billing ${!invoicesLoading ? `(${invoices.length})` : ""}` : "Overview"}</button>)}</div>{
    ...
    /* Next Appointment */
  }<div className="bg-white dark:bg-[#111111] border border-slate-200 dark:border-[#1e1e1e] rounded-lg p-4"><div className="flex items-center gap-1.5 text-xs text-slate-600 dark:text-[#555555] mb-3"><Calendar className="w-3.5 h-3.5" /> Next Appointment
                                    </div>{nextVisit ? <><p className="text-base font-bold text-slate-800 dark:text-[#f0f0f0]">{formatDate(nextVisit.apptDate)}, {nextVisit.apptTime.substring(0, 5)}</p><p className="text-xs text-slate-500 dark:text-[#666666] mt-1 font-semibold">{nextVisit.type.replace("_", " ")}</p><p className="text-xs text-slate-600 dark:text-[#555555] mt-0.5">
                                                Dr. {nextVisit.doctorName}</p></> : <p className="text-sm text-slate-600 dark:text-[#555555]">No upcoming visit</p>}</div>{
    /* Active Prescriptions */
  }<div className="bg-white dark:bg-[#111111] border border-slate-200 dark:border-[#1e1e1e] rounded-lg p-4"><div className="flex items-center gap-1.5 text-xs text-slate-600 dark:text-[#555555] mb-3"><FileText className="w-3.5 h-3.5" /> Prescriptions
  ...
  >
                                                View all <ChevronRight className="w-3 h-3" /></button></>}{prescriptions.length === 0 && <p className="text-xs text-slate-600 dark:text-[#555555] mt-1">None recorded</p>}</div>{
    /* Recent Lab Result */
  }<div className="bg-white dark:bg-[#111111] border border-slate-200 dark:border-[#1e1e1e] rounded-lg p-4"><div className="flex items-center gap-1.5 text-xs text-slate-600 dark:text-[#555555] mb-3"><Activity className="w-3.5 h-3.5" /> Recent Lab Result
    ...
    className="text-xs text-blue-500 hover:text-blue-600 dark:text-blue-400 mt-2 flex items-center gap-0.5"
  >
                                                View results <ChevronRight className="w-3 h-3" /></button></> : <p className="text-sm text-slate-600 dark:text-[#555555]">No lab results</p>}</div></div>{
    /* Recent Records */
  ...
  >
                                        View all <ChevronRight className="w-3 h-3" /></button></div>{recordsLoading ? <div className="flex justify-center py-10"><Loader2 className="w-6 h-6 animate-spin text-[#444444]" /></div> : records.length === 0 ? <div className="py-10 text-center"><ClipboardList className="w-8 h-8 text-slate-200 dark:text-[#2a2a2a] mx-auto mb-2" /><p className="text-sm text-slate-600 dark:text-[#555555]">No records yet</p></div> : <div className="divide-y divide-slate-100 dark:divide-transparent space-y-3 px-5 pb-5">{records.slice(0, 4).map((r) => {
    const meta = TYPE_META[r.historyType] ?? TYPE_META.OTHER;
    return <div key={r.id} className="flex gap-4 group"><div className="flex flex-col items-center"><div className={`w-2 h-2 rounded-full mt-2 shrink-0 ${meta.dot}`} /><div className="w-px flex-1 bg-slate-200 dark:bg-[#1e1e1e] my-1" /></div><div className="flex-1 pb-1"><div className="bg-white dark:bg-[#161616] border border-slate-200 dark:border-[#222222] rounded-lg p-4"><div className="flex items-start justify-between gap-3 mb-2"><span className={`text-[10px] font-bold uppercase tracking-wide px-2.5 py-1 rounded border ${meta.color} ${meta.darkColor}`}>{meta.label}</span><p className="text-xs text-slate-600 dark:text-[#555555] mt-0.5 flex items-center gap-1"><Clock className="w-3 h-3" />{formatDateTime(r.createdAt)}</p></div>{r.description && <p className="text-sm text-slate-600 dark:text-[#cccccc] truncate">{r.description}</p>}<div className="flex items-center gap-1.5 mt-2 text-xs text-slate-600 dark:text-[#555555]"><Stethoscope className="w-3.5 h-3.5" /><span>{r.createdBy.firstName} {r.createdBy.lastName}</span></div>{r.nextVisitDate && <div className="text-xs text-slate-900 dark:text-white dark:text-slate-300 mt-2 flex items-center gap-1.5 font-medium"><Calendar className="w-3.5 h-3.5" />
  ...
  />}{
    /* Timeline */
  }{recordsLoading ? <div className="flex justify-center py-16"><Loader2 className="w-7 h-7 animate-spin text-[#444444]" /></div> : records.length === 0 ? <div className="py-16 text-center"><ClipboardList className="w-10 h-10 text-slate-200 dark:text-[#282828] mx-auto mb-3" /><p className="text-sm font-semibold text-slate-500 dark:text-[#666666]">No records yet</p><p className="text-xs text-slate-600 dark:text-[#444444] mt-1">Add the first medical record above.</p></div> : <div>{records.map((r) => <RecordCard key={r.id} record={r} />)}</div>}</div>}{
    /* ── APPOINTMENTS TAB ── */
  }{tab === "appointments" && <div className="w-full max-w-5xl"><div className="flex items-center justify-between mb-5"><div><h3 className="font-semibold text-slate-800 dark:text-[#e5e5e5]">Appointments</h3><p className="text-xs text-slate-500 dark:text-[#666666] mt-0.5">{appointments.length} appointment{appointments.length !== 1 ? "s" : ""} for {patient.firstName}</p></div></div>{appointmentsLoading ? <div className="flex justify-center py-16"><Loader2 className="w-7 h-7 animate-spin text-[#444444]" /></div> : appointments.length === 0 ? <div className="py-16 text-center"><Calendar className="w-10 h-10 text-slate-200 dark:text-[#282828] mx-auto mb-3" /><p className="text-sm font-semibold text-slate-500 dark:text-[#666666]">No appointments yet</p><p className="text-xs text-slate-600 dark:text-[#444444] mt-1">Book an appointment from the Appointments Dashboard.</p></div> : <div className="grid grid-cols-1 md:grid-cols-2 gap-4">{appointments.map((appt) => <div key={appt.id} className="bg-white dark:bg-[#111111] border border-slate-200 dark:border-[#222222] rounded-lg p-5 hover:border-slate-300 dark:hover:border-[#333333] transition-colors relative overflow-hidden">{
    /* Status indicator line */
  }<div className={`absolute left-0 top-0 bottom-0 w-1 ${["COMPLETED"].includes(appt.status) ? "bg-slate-900 dark:bg-white" : ["CANCELLED", "NO_SHOW"].includes(appt.status) ? "bg-red-500" : ["IN_PROGRESS"].includes(appt.status) ? "bg-amber-500" : "bg-blue-500"}`} /><div className="flex justify-between items-start mb-4"><div><p className="text-lg font-bold text-slate-800 dark:text-white">{formatDate(appt.apptDate)}</p><p className="text-sm text-slate-500 dark:text-slate-600 mt-0.5 flex items-center gap-1.5 font-medium"><Clock className="w-3.5 h-3.5" />{appt.apptTime.substring(0, 5)} - {appt.apptEndTime ? appt.apptEndTime.substring(0, 5) : "Unknown"}</p></div><span className={`px-2.5 py-1 rounded text-xs font-bold uppercase ${["COMPLETED"].includes(appt.status) ? "bg-slate-100 dark:bg-[#1e1e1e] text-slate-900 dark:text-white dark:bg-slate-500/10 dark:text-slate-300" : ["CANCELLED", "NO_SHOW"].includes(appt.status) ? "bg-red-50 text-red-700 dark:bg-red-500/10 dark:text-red-400" : ["IN_PROGRESS"].includes(appt.status) ? "bg-amber-50 text-amber-700 dark:bg-amber-500/10 dark:text-amber-400" : "bg-blue-50 text-blue-700 dark:bg-blue-500/10 dark:text-blue-400"}`}>{appt.status.replace("_", " ")}</span></div><div className="space-y-3"><div className="flex items-center gap-2"><div className="w-8 h-8 rounded-full bg-slate-100 dark:bg-[#1a1a1a] flex items-center justify-center shrink-0"><Stethoscope className="w-4 h-4 text-slate-900 dark:text-white dark:text-slate-900 dark:text-white" /></div><div><p className="text-sm font-semibold text-slate-800 dark:text-slate-200">
                                                            Dr. {appt.doctorName}</p>{appt.doctorSpecialization && <p className="text-xs text-slate-500 dark:text-slate-600">{appt.doctorSpecialization}</p>}</div></div><div className="pt-3 border-t border-slate-100 dark:border-[#222222] grid grid-cols-2 gap-3"><div><p className="text-[10px] uppercase font-bold text-slate-600 dark:text-[#666666]">Type</p><p className="text-xs font-semibold text-slate-700 dark:text-slate-300 mt-0.5">{appt.type}</p></div>{appt.tokenNumber && <div><p className="text-[10px] uppercase font-bold text-slate-600 dark:text-[#666666]">Token No</p><p className="text-xs font-semibold text-slate-700 dark:text-slate-300 mt-0.5">
                                                                #{appt.tokenNumber}</p></div>}</div>{appt.chiefComplaint && <div className="bg-slate-50 dark:bg-[#161616] p-3 rounded-lg mt-2 border border-slate-100 dark:border-[#222222]"><p className="text-[10px] uppercase font-bold text-slate-600 dark:text-[#666666] mb-1 flex items-center gap-1"><FileText className="w-3 h-3" /> Reason for visit
    ...
    className="text-xs font-semibold px-3 py-1.5 rounded-lg bg-slate-900 dark:bg-white hover:bg-slate-900 dark:bg-white text-white transition-colors"
  >
                                    + New Invoice
                                </button></div>{invoicesLoading ? <div className="flex justify-center py-10"><Loader2 className="w-5 h-5 animate-spin text-slate-300" /></div> : invoices.length === 0 ? <div className="py-12 text-center"><p className="text-sm text-slate-600 dark:text-[#555555]">No invoices yet</p><button onClick={() => navigate(`/billing?patientId=${id}`)} className="mt-3 text-sm font-semibold text-slate-900 dark:text-white dark:text-slate-300 hover:underline">
                                        Create first invoice
                                    </button></div> : <div className="space-y-2">{invoices.map((inv) => {
    const statusCls = inv.status === "PAID" ? "bg-slate-100 dark:bg-[#1e1e1e] text-slate-900 dark:text-white border-emerald-200 dark:bg-slate-500/10 dark:text-slate-300 dark:border-slate-900 dark:border-white/20" : inv.status === "CANCELLED" ? "bg-red-50 text-red-700 border-red-200 dark:bg-red-500/10 dark:text-red-400 dark:border-red-500/20" : "bg-amber-50 text-amber-700 border-amber-200 dark:bg-amber-500/10 dark:text-amber-400 dark:border-amber-500/20";
    return <div key={inv.id} className="flex items-center justify-between px-4 py-3 rounded-lg border border-slate-100 dark:border-[#1e1e1e] hover:bg-slate-50 dark:hover:bg-[#0f0f0f] transition-colors"><div><p className="text-sm font-semibold text-slate-800 dark:text-[#dddddd]">#{inv.invoiceNumber}</p><p className="text-xs text-slate-600 dark:text-[#555555] mt-0.5">{inv.createdAt ? new Date(inv.createdAt).toLocaleDateString("en-IN") : ""}{inv.paymentMethod ? ` \xB7 ${inv.paymentMethod}` : ""}{" \xB7 "}{inv.items?.length ?? 0} item{(inv.items?.length ?? 0) !== 1 ? "s" : ""}</p></div><div className="flex items-center gap-3"><span className={`text-[11px] font-bold px-2 py-0.5 rounded-full border ${statusCls}`}>{inv.status}</span><span className="text-sm font-bold text-slate-800 dark:text-[#dddddd]">₹{inv.total.toLocaleString("en-IN", { minimumFractionDigits: 2 })}</span></div></div>;
  })}</div>}</div>}</div></div>{
    /* Edit modal */
