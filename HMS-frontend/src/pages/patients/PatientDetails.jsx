...
function PatientDetails() {
  ...
  >{t === "records" ? `Records ${!recordsLoading ? `(${records.length})` : ""}` : t === "appointments" ? `Appointments ${!appointmentsLoading ? `(${appointments.length})` : ""}` : t === "radiology" ? `Radiology ${!radiologyLoading ? `(${radiologyOrders.length})` : ""}` : t === "billing" ? `Billing ${!invoicesLoading ? `(${invoices.length})` : ""}` : "Overview"}</button>)}</div>{
    ...
    /* Next Appointment */
  }<div className="bg-white dark:bg-[#111111] border border-slate-200 dark:border-[#1e1e1e] rounded-lg p-4"><div className="flex items-center gap-1.5 text-xs text-slate-400 dark:text-[#555555] mb-3"><Calendar className="w-3.5 h-3.5" /> Next Appointment
                                    </div>{nextVisit ? <><p className="text-base font-bold text-slate-800 dark:text-[#f0f0f0]">{formatDate(nextVisit.apptDate)}, {nextVisit.apptTime.substring(0, 5)}</p><p className="text-xs text-slate-500 dark:text-[#666666] mt-1 font-semibold">{nextVisit.type.replace("_", " ")}</p><p className="text-xs text-slate-400 dark:text-[#555555] mt-0.5">
                                                Dr. {nextVisit.doctorName}</p></> : <p className="text-sm text-slate-400 dark:text-[#555555]">No upcoming visit</p>}</div>{
    /* Active Prescriptions */
  }<div className="bg-white dark:bg-[#111111] border border-slate-200 dark:border-[#1e1e1e] rounded-lg p-4"><div className="flex items-center gap-1.5 text-xs text-slate-400 dark:text-[#555555] mb-3"><FileText className="w-3.5 h-3.5" /> Prescriptions
  ...
  >
                                                View all <ChevronRight className="w-3 h-3" /></button></>}{prescriptions.length === 0 && <p className="text-xs text-slate-400 dark:text-[#555555] mt-1">None recorded</p>}</div>{
    /* Recent Lab Result */
  }<div className="bg-white dark:bg-[#111111] border border-slate-200 dark:border-[#1e1e1e] rounded-lg p-4"><div className="flex items-center gap-1.5 text-xs text-slate-400 dark:text-[#555555] mb-3"><Activity className="w-3.5 h-3.5" /> Recent Lab Result
    ...
    className="text-xs text-blue-500 hover:text-blue-600 dark:text-blue-400 mt-2 flex items-center gap-0.5"
  >
                                                View results <ChevronRight className="w-3 h-3" /></button></> : <p className="text-sm text-slate-400 dark:text-[#555555]">No lab results</p>}</div></div>{
    /* Recent Records */
  ...
  >
                                        View all <ChevronRight className="w-3 h-3" /></button></div>{recordsLoading ? <div className="flex justify-center py-10"><Loader2 className="w-6 h-6 animate-spin text-[#444444]" /></div> : records.length === 0 ? <div className="py-10 text-center"><ClipboardList className="w-8 h-8 text-slate-200 dark:text-[#2a2a2a] mx-auto mb-2" /><p className="text-sm text-slate-400 dark:text-[#555555]">No records yet</p></div> : <div className="divide-y divide-slate-100 dark:divide-transparent space-y-3 px-5 pb-5">{records.slice(0, 4).map((r) => {
    const meta = TYPE_META[r.historyType] ?? TYPE_META.OTHER;
    return <div key={r.id} className="flex gap-4 group"><div className="flex flex-col items-center"><div className={`w-2 h-2 rounded-full mt-2 shrink-0 ${meta.dot}`} /><div className="w-px flex-1 bg-slate-200 dark:bg-[#1e1e1e] my-1" /></div><div className="flex-1 pb-1"><div className="bg-white dark:bg-[#161616] border border-slate-200 dark:border-[#222222] rounded-lg p-4"><div className="flex items-start justify-between gap-3 mb-2"><span className={`text-[10px] font-bold uppercase tracking-wide px-2.5 py-1 rounded border ${meta.color} ${meta.darkColor}`}>{meta.label}</span><p className="text-xs text-slate-400 dark:text-[#555555] mt-0.5 flex items-center gap-1"><Clock className="w-3 h-3" />{formatDateTime(r.createdAt)}</p></div>{r.description && <p className="text-sm text-slate-600 dark:text-[#cccccc] truncate">{r.description}</p>}<div className="flex items-center gap-1.5 mt-2 text-xs text-slate-400 dark:text-[#555555]"><Stethoscope className="w-3.5 h-3.5" /><span>{r.createdBy.firstName} {r.createdBy.lastName}</span></div>{r.nextVisitDate && <div className="text-xs text-slate-900 dark:text-white dark:text-slate-300 mt-2 flex items-center gap-1.5 font-medium"><Calendar className="w-3.5 h-3.5" />
  ...
  >
    ...
    return <div key={inv.id} className="flex items-center justify-between px-4 py-3 rounded-xl border border-slate-100 dark:border-[#1e1e1e] hover:bg-slate-50 dark:hover:bg-[#0f0f0f] transition-colors"><div><p className="text-sm font-semibold text-slate-800 dark:text-[#dddddd]">#{inv.invoiceNumber}</p><p className="text-xs text-slate-400 dark:text-[#555555] mt-0.5">{inv.createdAt ? new Date(inv.createdAt).toLocaleDateString("en-IN") : ""}{inv.paymentMethod ? ` \xB7 ${inv.paymentMethod}` : ""}{" \xB7 "}{inv.items?.length ?? 0} item{(inv.items?.length ?? 0) !== 1 ? "s" : ""}</p></div><div className="flex items-center gap-3"><span className={`text-[11px] font-bold px-2 py-0.5 rounded-full border ${statusCls}`}>{inv.status}</span><span className="text-sm font-bold text-slate-800 dark:text-[#dddddd]">₹{inv.total.toLocaleString("en-IN", { minimumFractionDigits: 2 })}</span></div></div>;
  })}</div>}</div>}</div></div>{
    /* Edit modal */
