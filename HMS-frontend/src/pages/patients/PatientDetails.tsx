...
const TYPE_META: Record<string, { label: string; color: string; darkColor: string; dot: string }> = {
    CONSULTATION: {
        label: 'Consultation',
...
const BLOOD_DARK: Record<string, string> = {
    ...
    return (
        <div className="flex gap-4 group">
            ...
            <div className="flex flex-col items-center">
                ...
                <div className="w-px flex-1 bg-slate-200 dark:bg-[#1e1e1e] mt-1" />
            </div>
            {/* Card */}
            <div className="flex-1 pb-5">
                <div className="bg-white dark:bg-[#111111] border border-slate-200 dark:border-[#1e1e1e]
                    rounded-lg p-5 hover:border-slate-300 dark:hover:border-[#2a2a2a] transition-colors">
                    <div className="flex items-start justify-between gap-3 mb-3">
                        <span className={`text-[11px] font-semibold uppercase tracking-wide px-3 py-1 rounded-full border
                            ${meta.color} ${meta.darkColor}`}>
                            {meta.label}
                        </span>
                        <span className="text-xs text-slate-400 dark:text-[#555555] shrink-0 flex items-center gap-1.5">
                            <Clock className="w-3.5 h-3.5" />{formatDateTime(record.createdAt)}
                        </span>
                    </div>
                    {record.description && (
                        <p className="text-base font-semibold text-slate-800 dark:text-[#e0e0e0] leading-snug whitespace-pre-wrap mt-2">
                            {record.description}
                        </p>
                    )}
                    {record.nextVisitDate && (
                        <div className="flex items-center gap-1.5 mt-4 text-xs font-semibold text-emerald-600 dark:text-emerald-400">
                            ...
                            <span>Next visit: {formatDateTime(record.nextVisitDate)}</span>
                        </div>
                    )}
                    <div className="flex items-center gap-1.5 mt-3 text-xs text-slate-400 dark:text-[#555555]">
                        <Stethoscope className="w-4 h-4 text-slate-500" />
                        <span>
                            {record.createdBy.firstName} {record.createdBy.lastName}
                            <span className="text-slate-300 dark:text-[#444444]"> · {record.createdBy.role}</span>
                        </span>
                    </div>
                </div>
            </div>
...
function AddRecordForm({ patientId, hospitalId, onSaved, onCancel }: {
    patientId: number; hospitalId: string; onSaved: () => void; onCancel: () => void
}) {
    ...
    return (
        <div className="bg-white dark:bg-[#161616] border border-slate-200 dark:border-[#2a2a2a] rounded-lg p-5 mb-5">
            <div className="flex items-center justify-between mb-4">
                ...
                <button onClick={onCancel} className="text-slate-400 hover:text-slate-600 dark:hover:text-[#aaaaaa] transition-colors">
                    <X className="w-4 h-4" />
                </button>
            </div>
            <form onSubmit={handleSubmit} className="space-y-4">
                <div className="grid grid-cols-2 gap-4">
                    <div>
                        ...
                        <select className="input text-sm" value={form.historyType}
                            onChange={e => setForm(p => ({ ...p, historyType: e.target.value }))}>
                            {HISTORY_TYPES.map(t => <option key={t} value={t}>{t.replace('_', ' ')}</option>)}
                        </select>
                    </div>
                    <div>
                        <label className="label text-xs">Next Visit Date</label>
                ...
                <div>
                    <label className="label text-xs">Notes / Description</label>
...
export default function PatientDetails() {
    ...
    const [allocatedRoom, setAllocatedRoom] = useState<any>(null)
    ...
    const loadPatient = () => {
        ...
        patientApi.get(Number(id), user.hospitalId)
            .then(setPatient)
            .catch(() => notify('Failed to load patient', 'error'))
            .finally(() => setLoading(false))

        // Also fetch rooms to find if patient is allocated
        api.get(`/rooms?hospitalId=${user.hospitalId}`)
            .then(({ data }) => {
                const room = data.find((r: any) => r.currentPatient?.id === Number(id))
    ...
    const loadRecords = () => {
        ...
        recordApi.list(Number(id), user.hospitalId)
            .then(setRecords)
            .catch(console.error)
    ...
    const loadAppointments = () => {
        ...
        appointmentsApi.getByPatient(Number(id))
    ...
    const nextVisit = useMemo(() => {
        ...
        const future = appointments
            .filter(a => {
                ...
                // Keep if appointment date is exactly today or in the future
    ...
    if (loading) return (
        <div className="flex items-center justify-center h-64">
            <Loader2 className="w-8 h-8 animate-spin text-[#444444]" />
        </div>
    )
    if (!patient) return (
        <div className="flex flex-col items-center justify-center h-64 gap-3">
            <AlertCircle className="w-10 h-10 text-slate-300 dark:text-[#333333]" />
            <p className="text-slate-500 dark:text-[#666666]">Patient not found.</p>
            ...
            <aside className="w-72 shrink-0 flex flex-col bg-white dark:bg-[#111111] border-r border-slate-200 dark:border-[#1e1e1e] overflow-y-auto">
                ...
                <div className="px-5 pt-5 pb-3 border-b border-slate-200 dark:border-[#1e1e1e] flex justify-between items-center relative">
                    ...
                    <div className="relative">
                        ...
                        {menuOpen && (
                            <div className="absolute right-0 top-full mt-1 w-44 bg-white dark:bg-[#1a1a1a] border border-slate-200 dark:border-[#2a2a2a] rounded shadow-xl z-20 overflow-hidden">
                                <div className="px-3 py-2 text-xs font-semibold text-slate-500 dark:text-[#888888] border-b border-slate-100 dark:border-[#2a2a2a] uppercase tracking-wider">
                                    Actions
                                </div>
                                <button
                                ...
                                <button
                                    onClick={() => { setTab('records'); setShowAddRecord(true); setMenuOpen(false); }}
                                    className="w-full flex items-center gap-2.5 px-3 py-2.5 text-sm
                                        text-slate-700 dark:text-[#cccccc] hover:text-slate-900 hover:bg-slate-50 dark:hover:text-white dark:hover:bg-[#222222] transition-colors text-left"
                                >
                                ...
                                <button
                                    onClick={() => { navigate(`/billing?patientId=${patient.id}&name=${patient.firstName}+${patient.lastName}&mrn=${patient.mrn}`); setMenuOpen(false); }}
                                    className="w-full flex items-center gap-2.5 px-3 py-2.5 text-sm
                                        text-slate-700 dark:text-[#cccccc] hover:text-slate-900 hover:bg-slate-50 dark:hover:text-white dark:hover:bg-[#222222] transition-colors text-left"
                                >
                ...
                <div className="px-5 py-6 text-center border-b border-slate-200 dark:border-[#1e1e1e]">
                    <div className="w-16 h-16 rounded-full bg-slate-100 dark:bg-[#1e1e1e] border border-slate-200 dark:border-[#2a2a2a] mx-auto mb-3
                        flex items-center justify-center text-2xl font-bold text-slate-700 dark:text-white">
                    ...
                    {age !== null && (
                        <p className="text-sm text-slate-500 dark:text-[#888888] mt-0.5">{age} years · {patient.gender}</p>
                    )}
                    <div className="flex items-center justify-center gap-2 mt-3">
                        ...
                        {blood && (
                            <span className={`px-2.5 py-1 rounded-full text-xs font-semibold border
                                bg-red-50 text-red-700 border-red-200 ${bloodDarkClass}`}>
                                <Droplets className="inline w-3 h-3 mr-0.5" />{blood}
                            </span>
                        )}
                    </div>
                    <p className="text-xs text-slate-400 dark:text-[#444444] mt-3">{patient.mrn}</p>
                </div>
                    ...
                    {allocatedRoom && (
                        <div>
                            ...
                            <div className="bg-emerald-50 dark:bg-emerald-500/10 border border-emerald-200 dark:border-emerald-500/20 rounded-lg p-3">
                                <p className="text-xs font-semibold text-emerald-800 dark:text-emerald-400 mb-1">Current Room</p>
                                <div className="flex items-center gap-2">
                                    <p className="text-sm font-bold text-emerald-900 dark:text-emerald-300">{allocatedRoom.roomNumber}</p>
                                    <span className="text-[10px] uppercase font-bold px-1.5 py-0.5 rounded bg-white dark:bg-[#111111] text-emerald-700 dark:text-emerald-400 border border-emerald-200 dark:border-emerald-500/30">
                                        {allocatedRoom.roomType}
                                    </span>
                                </div>
                                {allocatedRoom.approxDischargeTime && (
                                    <p className="text-xs text-emerald-700 dark:text-emerald-500 mt-2 flex items-center gap-1.5">
                                        <CalendarClock className="w-3.5 h-3.5" />
                                        Est. Discharge: {new Date(allocatedRoom.approxDischargeTime).toLocaleDateString('en-IN', { month: 'short', day: 'numeric', hour: '2-digit', minute: '2-digit' })}
                                    </p>
                                )}
                            </div>
                        </div>
                        ...
                        <div className="space-y-5 w-full max-w-6xl">
                            ...
                            <div className="grid grid-cols-3 gap-4">
                                    ...
                                    {nextVisit ? (
                                        <>
                                            <p className="text-base font-bold text-slate-800 dark:text-[#f0f0f0]">
                                                {formatDate(nextVisit.apptDate)}, {nextVisit.apptTime.substring(0, 5)}
                                            </p>
                                            <p className="text-xs text-slate-500 dark:text-[#666666] mt-1 font-semibold">
                                                {nextVisit.type.replace('_', ' ')}
                                            </p>
                                        ...
                                        <p className="text-sm text-slate-400 dark:text-[#555555]">No upcoming visit</p>
                                    )}
                                </div>
                                ...
                                <div className="bg-white dark:bg-[#111111] border border-slate-200 dark:border-[#1e1e1e] rounded-lg p-4">
                                    ...
                                    <p className="text-base font-bold text-slate-800 dark:text-[#f0f0f0]">
                                        {recordsLoading ? '…' : prescriptions.length} Records
                                    </p>
                                    {prescriptions.length > 0 && (
                                        <>
                                            <p className="text-xs text-slate-500 dark:text-[#666666] mt-1">
                                    ...
                                    {prescriptions.length === 0 && (
                                        <p className="text-xs text-slate-400 dark:text-[#555555] mt-1">None recorded</p>
                                    )}
                                </div>
                                ...
                                <div className="bg-white dark:bg-[#111111] border border-slate-200 dark:border-[#1e1e1e] rounded-lg p-4">
                                    <div className="flex items-center gap-1.5 text-xs text-slate-400 dark:text-[#555555] mb-3">
                                        <Activity className="w-3.5 h-3.5" /> Recent Lab Result
                                    </div>
                                    {labResults[0] ? (
                                        <>
                                            <p className="text-sm font-semibold text-slate-800 dark:text-[#e0e0e0] leading-snug">
                                                {labResults[0].historyType.replace('_', ' ')}
                                            </p>
                                            <p className="text-xs text-slate-500 dark:text-[#666666] mt-1">
                                                {formatDate(labResults[0].createdAt)}
                                            </p>
                                            <button
                                                onClick={() => setTab('records')}
                                                className="text-xs text-blue-500 hover:text-blue-600 dark:text-blue-400 mt-2 flex items-center gap-0.5"
                                        ...
                                        <p className="text-sm text-slate-400 dark:text-[#555555]">No lab results</p>
                                    )}
                                </div>
                            </div>
                            ...
                            <div className="bg-white dark:bg-[#111111] border border-slate-200 dark:border-[#1e1e1e] rounded-lg overflow-hidden">
                                <div className="flex items-center justify-between px-5 py-4 border-b border-slate-100 dark:border-[#1e1e1e]">
                                    <h3 className="font-semibold text-slate-800 dark:text-[#e5e5e5] text-sm">Recent Records</h3>
                                    <button
                                        onClick={() => setTab('records')}
                                ...
                                {recordsLoading ? (
                                    <div className="flex justify-center py-10">
                                        <Loader2 className="w-6 h-6 animate-spin text-[#444444]" />
                                    </div>
                                ) : records.length === 0 ? (
                                    <div className="py-10 text-center">
                                        <ClipboardList className="w-8 h-8 text-slate-200 dark:text-[#2a2a2a] mx-auto mb-2" />
                                        <p className="text-sm text-slate-400 dark:text-[#555555]">No records yet</p>
                                    </div>
                                ) : (
                                    <div className="divide-y divide-slate-100 dark:divide-transparent space-y-3 px-5 pb-5">
                                            ...
                                            return (
                                                <div key={r.id} className="flex gap-4 group">
                                                    <div className="flex flex-col items-center">
                                                        ...
                                                        <div className="w-px flex-1 bg-slate-200 dark:bg-[#1e1e1e] my-1" />
                                                    </div>
                                                    <div className="flex-1 pb-1">
                                                        <div className="bg-white dark:bg-[#161616] border border-slate-200 dark:border-[#222222] rounded-lg p-4">
                                                            <div className="flex items-start justify-between gap-3 mb-2">
                                                                <span className={`text-[10px] font-bold uppercase tracking-wide px-2.5 py-1 rounded border ${meta.color} ${meta.darkColor}`}>
                                                                    {meta.label}
                                                                </span>
                                                                <p className="text-xs text-slate-400 dark:text-[#555555] mt-0.5 flex items-center gap-1">
                                                                    <Clock className="w-3 h-3" />{formatDateTime(r.createdAt)}
                                                                </p>
                                                            </div>
                                                            {r.description && (
                                                                <p className="text-sm text-slate-600 dark:text-[#cccccc] truncate">{r.description}</p>
                                                            )}
                                                            <div className="flex items-center gap-1.5 mt-2 text-xs text-slate-400 dark:text-[#555555]">
                                                                <Stethoscope className="w-3.5 h-3.5" />
                                                                <span>{r.createdBy.firstName} {r.createdBy.lastName}</span>
                                                            </div>
                                                            {r.nextVisitDate && (
                                                                <div className="text-xs text-emerald-500 dark:text-emerald-400 mt-2 flex items-center gap-1.5 font-medium">
                    ...
                    {tab === 'records' && (
                        <div className="w-full max-w-5xl">
                            ...
                            <div className="flex items-center justify-between mb-5">
                                <div>
                                    <h3 className="font-semibold text-slate-800 dark:text-[#e5e5e5]">Medical Records</h3>
                                    <p className="text-xs text-slate-500 dark:text-[#666666] mt-0.5">
                                        {records.length} record{records.length !== 1 ? 's' : ''} for {patient.firstName}
                                    </p>
                                </div>
                                <button
                            ...
                            {/* Inline add form */}
                            {showAddRecord && user?.hospitalId && (
                                <AddRecordForm
                                    patientId={patient.id}
                                    hospitalId={user.hospitalId}
                                    onSaved={() => { setShowAddRecord(false); loadRecords() }}
                            ...
                            {recordsLoading ? (
                                ...
                                <div className="py-16 text-center">
                                    <ClipboardList className="w-10 h-10 text-slate-200 dark:text-[#282828] mx-auto mb-3" />
                                    <p className="text-sm font-semibold text-slate-500 dark:text-[#666666]">No records yet</p>
                                    <p className="text-xs text-slate-400 dark:text-[#444444] mt-1">Add the first medical record above.</p>
                                </div>
                            ) : (
                                <div>
                        ...
                        <div className="w-full max-w-5xl">
                            <div className="flex items-center justify-between mb-5">
                                <div>
                                    ...
                                    <p className="text-xs text-slate-500 dark:text-[#666666] mt-0.5">
                                        {appointments.length} appointment{appointments.length !== 1 ? 's' : ''} for {patient.firstName}
                                    </p>
                                </div>
                            </div>
                            ...
                            {appointmentsLoading ? (
                                ...
                                <div className="py-16 text-center">
                                    <Calendar className="w-10 h-10 text-slate-200 dark:text-[#282828] mx-auto mb-3" />
                                    <p className="text-sm font-semibold text-slate-500 dark:text-[#666666]">No appointments yet</p>
                                    <p className="text-xs text-slate-400 dark:text-[#444444] mt-1">Book an appointment from the Appointments Dashboard.</p>
                                </div>
                            ) : (
                                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                                    {appointments.map(appt => (
                                        <div key={appt.id} className="bg-white dark:bg-[#111111] border border-slate-200 dark:border-[#222222] rounded-xl p-5 hover:border-slate-300 dark:hover:border-[#333333] transition-colors relative overflow-hidden">
                                            ...
                                            <div className={`absolute left-0 top-0 bottom-0 w-1 ${['COMPLETED'].includes(appt.status) ? 'bg-emerald-500' :
                                            ...
                                            <div className="flex justify-between items-start mb-4">
                                                <div>
                                                    ...
                                                    <p className="text-sm text-slate-500 dark:text-slate-400 mt-0.5 flex items-center gap-1.5 font-medium">
                                                        ...
                                                        {appt.apptTime.substring(0, 5)} - {appt.apptEndTime ? appt.apptEndTime.substring(0, 5) : 'Unknown'}
                                                    </p>
                                                </div>
                                                <span className={`px-2.5 py-1 rounded text-xs font-bold uppercase ${['COMPLETED'].includes(appt.status) ? 'bg-emerald-50 text-emerald-700 dark:bg-emerald-500/10 dark:text-emerald-400' :
                                            ...
                                            <div className="space-y-3">
                                                <div className="flex items-center gap-2">
                                                    <div className="w-8 h-8 rounded-full bg-slate-100 dark:bg-[#1a1a1a] flex items-center justify-center shrink-0">
                                                        <Stethoscope className="w-4 h-4 text-emerald-600 dark:text-emerald-500" />
                                                    </div>
                                                    <div>
                                                ...
                                                <div className="pt-3 border-t border-slate-100 dark:border-[#222222] grid grid-cols-2 gap-3">
                                                    <div>
                                                        <p className="text-[10px] uppercase font-bold text-slate-400 dark:text-[#666666]">Type</p>
                                                    ...
                                                    {appt.tokenNumber && (
                                                        <div>
                                                            <p className="text-[10px] uppercase font-bold text-slate-400 dark:text-[#666666]">Token No</p>
                                                            <p className="text-xs font-semibold text-slate-700 dark:text-slate-300 mt-0.5">
                                                                #{appt.tokenNumber}
                                                            </p>
                                                        </div>
                                                ...
                                                {appt.chiefComplaint && (
                                                    <div className="bg-slate-50 dark:bg-[#161616] p-3 rounded-lg mt-2 border border-slate-100 dark:border-[#222222]">
                                                        <p className="text-[10px] uppercase font-bold text-slate-400 dark:text-[#666666] mb-1 flex items-center gap-1">
