...
import { hospitalServiceApi, specializationApi, type HospitalService, type Specialization } from '@/utils/api'
import AddServiceModal from '@/components/modals/AddServiceModal'
import ServiceFilters, { type FilterState } from '@/components/specializations/ServiceFilters'
...
export default function Services() {
    ...
    const [modal, setModal] = useState<{ open: boolean; service: HospitalService | null }>({ open: false, service: null })
    ...
    return (
        <div className="p-6 space-y-6">
            ...
            <div className="flex justify-between items-center gap-4 bg-white/50 dark:bg-[#0a0a0a]/50 p-2 rounded-2xl border border-slate-100 dark:border-[#1a1a1a]">
                ...
                <div className="flex items-center gap-2 relative">
                    <button 
                        ...
                        className={`btn-secondary ${
                            isFilterOpen || activeFilters.departments.length > 0 || activeFilters.amountRange || activeFilters.statuses.length > 0
                    ...
                    <button className="btn-secondary">
                        Sort By : Recent <ChevronDown className="w-4 h-4 text-slate-400" />
                    </button>
                </div>
            </div>
            ...
            <div className="bg-white dark:bg-[#111111] border border-slate-200 dark:border-[#222222] rounded-2xl overflow-hidden shadow-sm">
                <div className="overflow-x-auto">
                    <table className="w-full text-left">
                        <thead>
                            <tr className="border-b border-slate-100 dark:border-[#1a1a1a] bg-slate-50/50 dark:bg-[#0a0a0a]/50">
                                <th className="px-6 py-4 text-xs font-bold text-slate-500 dark:text-[#555555] uppercase tracking-wider">Service Name</th>
                                <th className="px-6 py-4 text-xs font-bold text-slate-500 dark:text-[#555555] uppercase tracking-wider">Department</th>
                                <th className="px-6 py-4 text-xs font-bold text-slate-500 dark:text-[#555555] uppercase tracking-wider">Price</th>
                                <th className="px-6 py-4 text-xs font-bold text-slate-500 dark:text-[#555555] uppercase tracking-wider">Status</th>
                                <th className="px-10 py-4 text-xs font-bold text-slate-500 dark:text-[#555555] uppercase tracking-wider w-10"></th>
                            </tr>
                        </thead>
                        <tbody className="divide-y divide-slate-100 dark:divide-[#1a1a1a]">
                            {loading ? (
                                <tr>
                                    <td colSpan={5} className="px-6 py-12 text-center text-slate-400">Loading services...</td>
                                </tr>
                                ...
                                filteredServices.map(s => (
                                    <tr key={s.id} className="group hover:bg-slate-50/50 dark:hover:bg-[#161616]/50 transition-colors">
                                        <td className="px-6 py-4">
                                            <p className="font-semibold text-slate-700 dark:text-[#cccccc] text-sm group-hover:text-indigo-600 dark:group-hover:text-indigo-400 transition-colors">
                                                {s.name}
                                            </p>
                                        </td>
                                        <td className="px-6 py-4">
                                            <span className="px-2.5 py-1 bg-slate-100 dark:bg-[#1e1e1e] text-slate-600 dark:text-[#888888] text-xs font-medium rounded-lg border border-slate-200 dark:border-[#2a2a2a]">
                                        ...
                                        <td className="px-6 py-4 relative">
                                            ...
                                            {openMenuId === s.id && (
                                                <div className="absolute right-6 top-12 w-44 bg-white dark:bg-[#1a1a1a] border border-slate-200 dark:border-[#2a2a2a] rounded-xl shadow-xl z-20 py-1.5 animate-in fade-in zoom-in duration-150">
                                                    <button 
