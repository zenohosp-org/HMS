    ...
    return (
        <div className="space-y-5">
            <div className="flex items-center justify-between">
                <div>
                    <h1 className="text-xl font-bold text-slate-900 dark:text-[#f0f0f0]">Manage Staff</h1>
                    <p className="text-sm text-slate-500 dark:text-[#666666]">{staff.length} total accounts</p>
                </div>
                <button className="btn-primary" onClick={() => setShowModal(true)}>+ Add Doctor / Staff</button>
            </div>
            ...
            <div className="space-y-3">
                {loading ? (
                    ...
                    <div className="bg-white dark:bg-[#111111] border border-slate-200 dark:border-[#1e1e1e] rounded-xl p-8 text-center">
                        <p className="text-slate-500 dark:text-[#666666]">No accounts found.</p>
                    </div>
                ) : (
                    visible.map(s => {
                            ...
                            >
                                <div className="flex items-center gap-4">
                                    ...
                                    <div>
                                        ...
                                        <p className="text-xs text-slate-500 dark:text-[#666666] mt-0.5">{s.email}</p>
                                    </div>
                                </div>
                                <div className="flex items-center gap-4">
                                    <div className="text-right hidden sm:block mr-4">
                                        <p className="text-sm font-semibold text-slate-700 dark:text-[#cccccc]">{s.roleDisplay}</p>
                                        {s.specialization && (
                                            <p className="text-xs text-slate-500 dark:text-[#666666] mt-0.5">{s.specialization}</p>
                                        )}
                                    </div>
