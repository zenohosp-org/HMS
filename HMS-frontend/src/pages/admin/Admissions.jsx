              ...
              {a.status === 'ADMITTED' && (
                <div className="px-4 pb-4 flex gap-2" onClick={e => e.stopPropagation()}>
                  <button onClick={() => setOtTarget(a)}
                    className="flex-1 flex items-center justify-center gap-1.5 py-2 rounded-lg text-xs font-semibold bg-slate-100 dark:bg-[#1e1e1e] text-slate-900 dark:text-white dark:text-slate-300 hover:bg-slate-100 dark:hover:bg-slate-200 border border-slate-200 dark:border-[#333333] transition-colors">
                ...
                <tr key={a.id} onClick={() => navigate(`/patients/${a.patientId}`)} className={`hover:bg-slate-50 dark:hover:bg-[#161616] transition-colors cursor-pointer ${isOverdue(a) ? 'border-l-4 border-l-rose-400' : ''}`}>
                  ...
                  <td className="px-4 py-3" onClick={e => e.stopPropagation()}>
                    {a.status === 'ADMITTED' && (
                      <div className="flex items-center gap-3">
                        <button onClick={() => setOtTarget(a)}
                          className="flex items-center gap-1 text-xs font-semibold text-slate-900 dark:text-white hover:text-slate-900 dark:text-white dark:text-slate-300 transition-colors">
