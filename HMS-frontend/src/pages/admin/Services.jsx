  ...
  return <div className="space-y-6">
    <div className="flex justify-between items-center">
      <div className="flex items-center gap-3">
        ...
        <span className="px-2 py-0.5 bg-indigo-50 dark:bg-indigo-500/10 text-indigo-600 dark:text-indigo-400 text-xs font-semibold rounded-full border border-indigo-100 dark:border-indigo-500/20">
    ...
    <div className="flex justify-between items-center gap-4">
      <div className="relative flex-1">
        ...
        <input type="text" placeholder="Search by service name or department..."
          value={search}
          onChange={(e) => setSearch(e.target.value)}
          className="w-full pl-10 pr-4 py-2 text-sm bg-white dark:bg-[#111111] border border-slate-300 dark:border-[#222222] rounded-lg focus:ring-2 focus:ring-indigo-500/20 outline-none transition-all placeholder-slate-400 dark:placeholder-[#444444] text-slate-700 dark:text-[#cccccc]"
        />
      </div>
      <div className="flex items-center gap-2 relative">
        ...
        >
          <Filter className={`w-4 h-4 transition-colors ${isFilterOpen || activeFilters.departments.length > 0 || activeFilters.amountRange || activeFilters.statuses.length > 0 ? "text-indigo-600 dark:text-indigo-400" : "text-slate-600 group-hover:text-indigo-500"}`} />
          Filters
        </button>
        <ServiceFilters
          isOpen={isFilterOpen}
    ...
    <div className="bg-white dark:bg-[#111111] border border-slate-200 dark:border-[#222222] rounded-lg overflow-hidden shadow-sm"><div className="overflow-x-auto"><table className="w-full text-left"><thead><tr className="border-b border-slate-100 dark:border-[#1a1a1a] bg-slate-50/50 dark:bg-[#0a0a0a]/50"><th className="px-6 py-4 text-xs font-bold text-slate-500 dark:text-[#999999] uppercase tracking-wider">Service Name</th><th className="px-6 py-4 text-xs font-bold text-slate-500 dark:text-[#999999] uppercase tracking-wider">Department</th><th className="px-6 py-4 text-xs font-bold text-slate-500 dark:text-[#999999] uppercase tracking-wider">Price</th><th className="px-6 py-4 text-xs font-bold text-slate-500 dark:text-[#999999] uppercase tracking-wider">Status</th><th className="px-10 py-4 text-xs font-bold text-slate-500 dark:text-[#999999] uppercase tracking-wider w-10" /></tr></thead><tbody className="divide-y divide-slate-100 dark:divide-[#1a1a1a]">{loading ? <tr><td colSpan={5} className="px-6 py-12 text-center text-slate-600">Loading services...</td></tr> : filteredServices.length === 0 ? <tr><td colSpan={5} className="px-6 py-12 text-center text-slate-600">No services found.</td></tr> : filteredServices.map((s) => <tr key={s.id} className="group hover:bg-slate-50/50 dark:hover:bg-[#161616]/50 transition-colors"><td className="px-6 py-4"><p className="font-semibold text-slate-700 dark:text-[#cccccc] text-sm group-hover:text-indigo-600 dark:group-hover:text-indigo-400 transition-colors">{s.name}</p></td><td className="px-6 py-4"><span className="px-2.5 py-1 bg-slate-100 dark:bg-[#1e1e1e] text-slate-600 dark:text-[#888888] text-xs font-medium rounded-lg border border-slate-200 dark:border-[#2a2a2a]">{getSpecName(s.specializationId)}</span></td><td className="px-6 py-4"><p className="font-bold text-slate-900 dark:text-white text-sm">
      ₹{s.price}</p></td><td className="px-6 py-4"><span className={`inline-flex items-center px-2.5 py-0.5 rounded-full text-[10px] font-bold uppercase tracking-wider border ${s.isActive ? "bg-slate-100 dark:bg-[#1e1e1e] dark:bg-slate-500/10 text-slate-900 dark:text-white dark:text-slate-500 border-emerald-100 dark:border-slate-900 dark:border-white/20" : "bg-rose-50 dark:bg-rose-500/10 text-rose-600 dark:text-rose-400 border-rose-100 dark:border-rose-500/20"}`}>{s.isActive ? "Active" : "Inactive"}</span></td><td className="px-6 py-4 relative"><button
        onClick={() => setOpenMenuId(openMenuId === s.id ? null : s.id)}
        className="p-1.5 hover:bg-slate-100 dark:hover:bg-[#222222] rounded-lg text-slate-600 transition-colors"
      >
        <MoreVertical className="w-4 h-4" /></button>{openMenuId === s.id && <div className="absolute right-6 top-12 w-44 bg-white dark:bg-[#1a1a1a] border border-slate-200 dark:border-[#2a2a2a] rounded-lg shadow-xl z-20 py-1.5 animate-in fade-in zoom-in duration-150"><button
          onClick={() => {
