...
function Services() {
  ...
  return <div className="space-y-6"><div className="flex justify-between items-center"><div className="flex items-center gap-3"><h1 className="text-2xl font-bold text-slate-800 dark:text-white">Services</h1><span className="px-2 py-0.5 bg-slate-100 dark:bg-slate-100 text-slate-900 dark:text-white dark:text-slate-400 text-xs font-semibold rounded-full border border-slate-200 dark:border-slate-200/20">
    Total Services : {services.length}</span></div><div className="flex gap-3"><button className="btn-secondary">
      ...
      ><Plus className="w-4 h-4" /> New Service
        ...
        className="w-full pl-10 pr-4 py-2 text-sm bg-white dark:bg-[#111111] border border-slate-200 dark:border-[#222222] rounded-lg focus:ring-2 focus:ring-slate-300/20 outline-none transition-all placeholder-slate-400 dark:placeholder-[#444444] text-slate-700 dark:text-[#cccccc]"
      /></div><div className="flex items-center gap-2 relative"><button
        onClick={() => setIsFilterOpen(!isFilterOpen)}
        className={`btn-secondary ${isFilterOpen || activeFilters.departments.length > 0 || activeFilters.amountRange || activeFilters.statuses.length > 0 ? "bg-slate-100 dark:bg-slate-100 text-slate-900 dark:text-white dark:text-slate-400 border-slate-200 dark:border-slate-200/30" : ""}`}
      ><Filter className={`w-4 h-4 transition-colors ${isFilterOpen || activeFilters.departments.length > 0 || activeFilters.amountRange || activeFilters.statuses.length > 0 ? "text-slate-900 dark:text-white dark:text-slate-400" : "text-slate-400 group-hover:text-slate-700"}`} />
        Filters
