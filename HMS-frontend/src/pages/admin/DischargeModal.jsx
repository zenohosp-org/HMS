...
import {
  ...
  Pill, FlaskConical, Wrench, Landmark, Scissors,
} from 'lucide-react'
...
const TYPE_META = {
  ...
  RADIOLOGY:    { label: 'Radiology',    icon: ScanLine,     color: 'text-slate-900 dark:text-slate-300',    bg: 'bg-slate-100 dark:bg-[#1e1e1e]'       },
  ...
  OT:           { label: 'OT / Surgery', icon: Scissors,     color: 'text-purple-600 dark:text-purple-400', bg: 'bg-purple-50 dark:bg-purple-500/10'   },
...
export default function DischargeModal({ admission, onClose, onDischarged }) {
  ...
  useEffect(() => {
    ...
    Promise.all([
      ...
      .then(([suggestions, services, accounts, fullAdmission, radiologyOrders, patientServices, otRes]) => {
        ...
        // ── OT charges — pulled from OT app, scoped to this admission ────────
        const otInvoiceList = Array.isArray(otRes?.data) ? otRes.data : []
        otInvoiceList.forEach(inv => {
    ...
    <>
      <div className="fixed inset-0 z-50 flex bg-white dark:bg-[#111] overflow-hidden">
        ...
        <div className="flex flex-col flex-1 min-w-0 h-full border-r border-slate-200 dark:border-[#2a2a2a]">
          ...
          {/* Footer */}
          <div className="shrink-0 flex items-center justify-between gap-3 px-6 py-4 border-t border-slate-100 dark:border-[#1e1e1e] bg-slate-50 dark:bg-[#0a0a0a]">
            {step === 1 ? (
              <>
        ...
        <div className="w-[360px] shrink-0 bg-slate-50 dark:bg-[#0a0a0a] overflow-y-auto p-6 space-y-5">
          <div>
          ...
          {items.length > 0 && (
            <div className="rounded-xl border border-slate-200 dark:border-[#2a2a2a] bg-white dark:bg-[#111] overflow-hidden">
              <div className="px-4 py-3 border-b border-slate-100 dark:border-[#1a1a1a]">
                <p className="text-[10px] font-bold uppercase tracking-wider text-slate-400 dark:text-[#666]">Bill Summary</p>
              </div>
              <div className="divide-y divide-slate-50 dark:divide-[#141414]">
                {Object.entries(
                  items.reduce((acc, i) => {
                    const label = TYPE_META[i.itemType]?.label ?? i.itemType
                    acc[label] = (acc[label] || 0) + (i.totalPrice || 0)
                    return acc
                  }, {})
                ).map(([label, total]) => (
                  <div key={label} className="flex justify-between items-center px-4 py-2.5">
                    <span className="text-xs text-slate-600 dark:text-[#aaa]">{label}</span>
                    <span className="text-xs font-semibold text-slate-800 dark:text-white tabular-nums">{fmt(total)}</span>
                  </div>
                ))}
              </div>
              <div className="flex justify-between items-center px-4 py-3 border-t border-slate-100 dark:border-[#1a1a1a] bg-slate-50 dark:bg-[#0a0a0a]">
                <span className="text-xs font-bold text-slate-700 dark:text-white">Grand Total</span>
                <span className="text-sm font-bold text-blue-600 dark:text-blue-400 tabular-nums">{fmt(grandTotal)}</span>
              </div>
            </div>
          )}
        </div>
