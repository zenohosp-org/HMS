...
function DoctorsList() {
  ...
  return <div className="flex flex-col h-full bg-slate-50 dark:bg-[#050505] gap-6">{
    ...
    /* Search */
  }<div className="relative max-w-sm"><Search className="absolute left-3.5 top-1/2 -translate-y-1/2 w-4 h-4 text-slate-600" /><input
    type="text"
  ...
  /></div>{
    ...
    >
                                                        Dr. {d.firstName} {d.lastName}</button><p className="text-xs text-slate-600 dark:text-slate-500 mt-0.5">{d.qualification || "N/A"}</p></div></div></td><td className="px-6 py-4 text-sm font-medium text-slate-600 dark:text-slate-600">{d.specialization || "General"}</td><td className="px-6 py-4"><p className="text-sm text-slate-600 dark:text-slate-600">{d.email}</p><p className="text-xs text-slate-600 dark:text-slate-500 mt-0.5">{d.phone}</p></td><td className="px-6 py-4"><span className={`inline-flex items-center px-2.5 py-1 rounded-lg text-xs font-bold border ${d.userIsActive ? "bg-emerald-50 text-emerald-700 dark:bg-emerald-500/10 dark:text-emerald-400 border-emerald-200 dark:border-emerald-500/20" : "bg-rose-50 text-rose-700 border-rose-200 dark:bg-rose-900/10 dark:text-rose-400 dark:border-rose-800/30"}`}>{d.userIsActive ? "Active" : "Inactive"}</span></td><td className="px-6 py-4 text-right relative"><button
      onClick={(e) => {
        e.stopPropagation();
