  ...
  return (
    <div className="flex flex-col h-full bg-slate-50 dark:bg-[#0d0d0d] gap-6">
      ...
      <div className="flex gap-2">
        {DEPT_TYPES.map(t => (
          <button key={t} onClick={() => setActiveTab(t)}
            className={`px-4 py-2 rounded-lg text-sm font-semibold transition-all ${activeTab === t ? 'bg-slate-900 text-white dark:bg-white dark:text-slate-900' : 'bg-white dark:bg-[#111111] border border-slate-200 dark:border-[#222222] text-slate-600 dark:text-[#aaaaaa] hover:bg-slate-100 dark:hover:bg-[#1a1a1a]'}`}>
      ...
      <div className="rounded-lg bg-white dark:bg-[#111111] border border-slate-200 dark:border-[#1e1e1e] overflow-hidden flex-1">
          ...
          <table className="w-full">
            ...
            <tbody className="divide-y divide-slate-100 dark:divide-[#1e1e1e]">
              ...
              {grouped[activeTab]?.map(dept => (
                <tr key={dept.id} className="group hover:bg-slate-50 dark:hover:bg-[#1a1a1a] transition-colors border-b border-slate-100 dark:border-[#1e1e1e] last:border-0">
                  ...
                  <td className="px-5 py-3.5">
                    <span className={`px-2.5 py-1 rounded-lg text-xs font-semibold border ${TYPE_COLORS[dept.type]}`}>
                  ...
                  <td className="px-5 py-3.5">
                    <div className="flex items-center gap-2 justify-end">
                      <button onClick={() => openEdit(dept)} className="p-1.5 rounded-lg text-slate-400 dark:text-[#666] hover:text-slate-700 dark:hover:text-white hover:bg-slate-100 dark:hover:bg-[#1e1e1e] border border-transparent hover:border-slate-200 dark:hover:border-[#333] transition-all">
                        <Pencil className="w-3.5 h-3.5" />
                      </button>
                      <button onClick={() => toggle(dept)} className="p-1.5 rounded-lg text-slate-400 dark:text-[#666] hover:text-slate-700 dark:hover:text-white hover:bg-slate-100 dark:hover:bg-[#1e1e1e] border border-transparent hover:border-slate-200 dark:hover:border-[#333] transition-all">
        ...
        <div className="mt-6 rounded-lg border border-dashed border-slate-300 dark:border-[#333333] bg-slate-50/50 dark:bg-[#111111] p-4">
          <div className="flex items-center gap-3 mb-3"><p className="text-[10px] font-bold uppercase tracking-widest text-slate-400 dark:text-[#666]">Quick Add from Presets</p><div className="flex-1 h-px bg-slate-200 dark:bg-[#222222]"></div></div>
          <div className="flex flex-wrap gap-2">
            {PRESETS[activeTab]?.filter(p => !existing.has(p.name)).map(p => (
              <button key={p.name} onClick={() => openCreate(p)}
                className="group flex items-center gap-1.5 px-3 py-1.5 rounded-lg text-xs font-semibold bg-white dark:bg-[#1a1a1a] border border-slate-200 dark:border-[#2a2a2a] text-slate-600 dark:text-[#aaaaaa] hover:bg-slate-900 hover:text-white hover:border-slate-900 dark:hover:bg-white dark:hover:text-slate-900 dark:hover:border-white transition-all duration-150 shadow-sm">
                <Plus className="w-3 h-3 text-slate-400 group-hover:text-white dark:group-hover:text-slate-900 transition-colors" /> {p.name}
              </button>
            ))}
