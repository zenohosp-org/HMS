...
const AVATAR_CLS = {
  ...
  staff: "bg-slate-100 text-slate-900 dark:text-white border-slate-200 dark:bg-[#1e1e1e] dark:text-slate-300 dark:border-[#333333]"
};
const ROLE_BADGE = {
  doctor: "bg-blue-50 text-blue-700 border-blue-200 dark:bg-blue-500/10 dark:text-blue-400 dark:border-blue-500/20",
  hospital_admin: "bg-rose-50 text-rose-700 border-rose-200 dark:bg-rose-500/10 dark:text-rose-400 dark:border-rose-500/20",
  ...
  ><UserPlus className="w-4 h-4" /> Add Member
      ...
      <div className="flex gap-1.5">{ROLE_TABS.map((tab) => <button
        ...
        className={`px-3 py-2 rounded-lg text-sm font-semibold transition-colors
                                ${roleFilter === tab.key ? "bg-slate-900 dark:bg-white text-white dark:text-slate-900" : "text-slate-500 dark:text-[#888888] hover:bg-slate-100 dark:hover:bg-[#1a1a1a]"}`}>{tab.label}</button>)}
      </div>
      ...
      /* Cards grid */
    }{loading ? <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-4">{Array.from({ length: 8 }).map((_, i) => <div key={i} className="bg-white dark:bg-[#111111] border border-slate-200 dark:border-[#1e1e1e] rounded-lg p-5 animate-pulse"><div className="flex items-center gap-3 mb-4"><div className="w-12 h-12 rounded-full bg-slate-100 dark:bg-[#222222]" /><div className="flex-1 space-y-2"><div className="h-3 bg-slate-100 dark:bg-[#222222] rounded w-3/4" /><div className="h-2.5 bg-slate-100 dark:bg-[#222222] rounded w-1/2" /></div></div></div>)}</div> : filtered.length === 0 ? <div className="bg-white dark:bg-[#111111] border border-slate-200 dark:border-[#1e1e1e] rounded-lg p-12 text-center"><Users className="w-10 h-10 text-slate-300 dark:text-[#333333] mx-auto mb-3" /><p className="text-sm text-slate-500 dark:text-[#666666]">No members found</p>{search && <p className="text-xs text-slate-600 dark:text-[#999999] mt-1">Try clearing your search</p>}</div> : <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-4">{filtered.map((m) => {
      ...
      >{
          ...
          /* Details */
        }<div className="space-y-1.5 text-xs text-slate-500 dark:text-[#666666]">{m.designation && <p className="font-semibold text-slate-700 dark:text-[#aaaaaa] truncate">{m.designation}</p>}{m.departmentName && <p className="text-[10px] text-slate-900 dark:text-white dark:text-slate-300 font-medium truncate">{m.departmentName}</p>}<div className="flex items-center gap-1.5 truncate"><Mail className="w-3 h-3 shrink-0" /><span className="truncate">{m.email}</span></div>{m.phone && <div className="flex items-center gap-1.5"><Phone className="w-3 h-3 shrink-0" /><span>{m.phone}</span></div>}{m.dateOfJoining && <div className="flex items-center gap-1.5"><Calendar className="w-3 h-3 shrink-0" /><span>Joined {m.dateOfJoining}</span></div>}{m.consultationFee != null && <p className="font-semibold text-emerald-600 dark:text-emerald-400">
