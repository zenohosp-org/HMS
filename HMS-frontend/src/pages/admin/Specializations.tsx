...
import { Plus, Search, Filter, MoreHorizontal, Edit, Trash2, Loader2, Stethoscope, ChevronRight } from 'lucide-react'
import { specializationApi, type Specialization } from '@/utils/api'
...
export default function Specializations() {
    ...
    const [activeFilters, setActiveFilters] = useState<{ date: string | null; statuses: string[] }>({
        date: null,
        statuses: []
    })
    ...
    const filteredSpecs = useMemo(() => {
        return specializations.filter(s => {
            ...
            const matchesDate = !activeFilters.date || 
                isSameDay(parseISO(s.createdAt), parseISO(activeFilters.date))

            // Status filter
            const matchesStatus = activeFilters.statuses.length === 0 || 
                (s.isActive && activeFilters.statuses.includes('Active')) ||
                (!s.isActive && activeFilters.statuses.includes('Inactive'))
    ...
    const toggleStatus = async (spec: Specialization) => {
        try {
        ...
        <div className="flex flex-col h-full bg-slate-50 dark:bg-[#050505] p-6 gap-6">
            ...
            <div className="flex flex-col gap-1">
                <div className="flex items-center justify-between">
                    <div className="flex items-center gap-3">
                        <h1 className="text-2xl font-bold text-slate-900 dark:text-white tracking-tight">Specializations</h1>
            ...
            <div className="flex items-center justify-between gap-4">
                ...
                <div className="flex items-center gap-2">
                    <div className="relative">
                        <button 
                            ...
                            className={`btn-secondary ${
                                isFilterOpen || activeFilters.date || activeFilters.statuses.length > 0
                                ? 'bg-emerald-50 dark:bg-emerald-900/10 text-emerald-600 border-emerald-100 dark:border-emerald-800/30'
                                : ''
                            }`}
                        >
                            <Filter className="w-4 h-4" />
                            Filters
                            {(activeFilters.date || activeFilters.statuses.length > 0) && (
                    ...
                    <div className="flex items-center gap-2 px-4 py-2 border border-slate-200 dark:border-[#222222] rounded-xl text-sm font-semibold text-slate-600 dark:text-slate-400 bg-white dark:bg-[#111111] shadow-sm">
                        <span>Sort By :</span>
                        <select className="bg-transparent outline-none cursor-pointer text-slate-900 dark:text-white">
                                ...
                                <tr>
                                    <td colSpan={5} className="py-20 text-center">
                                        <div className="flex flex-col items-center gap-3">
                                            ...
                                            <p className="text-sm font-medium text-slate-400">Loading specializations...</p>
                                        </div>
                                    </td>
                                </tr>
                                ...
                                <tr>
                                    <td colSpan={5} className="py-20 text-center">
                                        <div className="flex flex-col items-center gap-3">
                                            ...
                                            <p className="text-sm font-medium text-slate-400">No specializations found.</p>
                                        </div>
                                    </td>
                                </tr>
                            ) : filteredSpecs.map((spec) => (
                                <tr key={spec.id} className="group hover:bg-slate-50/50 dark:hover:bg-[#151515] transition-all">
                                    <td className="px-6 py-4">
                                        <div className="flex items-center gap-4">
                                            <div className="w-11 h-11 rounded-full bg-slate-100 dark:bg-[#222222] flex items-center justify-center shrink-0">
                                                <Stethoscope className="w-5 h-5 text-slate-500 dark:text-slate-400" />
                                            </div>
                                            <div>
                                                <p className="font-bold text-slate-900 dark:text-white text-[15px]">{spec.name}</p>
                                                {spec.description && (
                                                    <p className="text-xs text-slate-500 dark:text-slate-500 mt-0.5 line-clamp-1 max-w-[200px]">{spec.description}</p>
                                                )}
                                            </div>
                                        </div>
                                    </td>
                                    <td className="px-6 py-4 text-sm font-medium text-slate-600 dark:text-slate-400">
                                        {format(parseISO(spec.createdAt), 'dd MMM yyyy')}
                                    </td>
                                    ...
                                    <td className="px-6 py-4">
                                        <button 
                                            ...
                                            className={`inline-flex items-center px-2.5 py-1 rounded-lg text-xs font-bold border transition-all ${
                                                spec.isActive 
                                    ...
                                    <td className="px-6 py-4 text-right relative">
                                        ...
                                        {activeMenuId === spec.id && (
                                            <>
                                                ...
                                                />
                                                <div className="absolute right-6 top-14 w-44 bg-white dark:bg-[#1a1a1a] rounded-xl shadow-xl border border-slate-100 dark:border-[#252525] z-20 py-1.5 animate-in slide-in-from-top-2 duration-150">
                                                    <button 
                                                        onClick={() => { setEditingSpec(spec); setIsModalOpen(true); setActiveMenuId(null) }}
                                                    ...
                                                    <button 
                                                        onClick={() => { handleDelete(spec.id); setActiveMenuId(null) }}
