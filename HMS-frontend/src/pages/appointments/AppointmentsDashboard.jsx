...
import { Calendar as CalendarIcon, Filter, Plus, ChevronLeft, ChevronRight, MoreHorizontal, CheckCircle2, XCircle, AlertCircle, LogIn, Loader2, PlayCircle } from "lucide-react";
...
function AppointmentsDashboard() {
  ...
  ><Plus className="w-4 h-4" />
    ...
    className={`px-4 py-2 text-sm font-semibold rounded-xl capitalize transition-all ${listFilter === f ? "bg-slate-950 text-white dark:bg-white dark:text-slate-950 shadow-md" : "text-slate-600 dark:text-[#888888] hover:bg-slate-100 dark:hover:bg-[#222222]"}`}
  >{f === "all" ? "All Appointments" : f}</button>)}</div>}</header>{
    /* Content Area */
  }<div className="flex-1 p-6 overflow-hidden flex flex-col gap-6">{viewMode === "calendar" && <div className="flex items-center justify-between pb-2"><div className="flex items-center bg-white dark:bg-[#111111] rounded-xl p-1 shadow-sm border border-slate-200 dark:border-[#333333]">{["day", "week", "month"].map((v) => <button
    key={v}
    onClick={() => setCalendarView(v)}
    className={`px-4 py-1.5 text-sm font-semibold rounded-xl capitalize transition-all ${calendarView === v ? "bg-white dark:bg-white dark:text-slate-950 text-slate-950 shadow-md" : "text-slate-500 dark:text-[#888888] hover:text-slate-700 dark:hover:text-[#cccccc]"}`}
