...
function RadiologyReports() {
  ...
  /></div></div>{
    /* Reports table */
  }<div className="bg-white dark:bg-[#111111] border border-slate-200 dark:border-[#1e1e1e] rounded-xl overflow-hidden border-t-4 border-violet-400"><div className="px-6 py-4 border-b border-slate-100 dark:border-[#1e1e1e]"><p className="text-sm font-bold text-slate-800 dark:text-[#dddddd]">Completed Reports</p><p className="text-xs text-slate-400 dark:text-[#555555] mt-0.5">Radiology reports with findings</p></div>{loading ? <div className="flex items-center justify-center py-16"><Loader2 className="w-5 h-5 animate-spin text-slate-400" /></div> : filtered.length === 0 ? <div className="flex flex-col items-center justify-center py-16 gap-2 text-slate-400 dark:text-[#555555]"><FileText className="w-8 h-8 opacity-30" /><p className="text-sm">{search ? "No reports match your search" : "No completed reports yet"}</p></div> : <>{
    /* Column headers */
  }<div className="hidden md:grid grid-cols-[2.5fr_2fr_2fr_1.5fr_1fr_auto] gap-4 px-6 py-2.5 bg-slate-50 dark:bg-[#0d0d0d] border-b border-slate-100 dark:border-[#1e1e1e]">{["Patient", "Investigation", "Referred By", "Completed", "Priority", "Action"].map((h) => <p key={h} className="text-[10px] font-bold uppercase tracking-widest text-slate-400 dark:text-[#555555]">{h}</p>)}</div><div className="divide-y divide-slate-100 dark:divide-[#1a1a1a]">{filtered.slice((page - 1) * PAGE_SIZE, page * PAGE_SIZE).map((order) => <div key={order.id} className="px-6 py-4 hover:bg-slate-50 dark:hover:bg-[#151515] transition-colors md:grid md:grid-cols-[2.5fr_2fr_2fr_1.5fr_1fr_auto] md:gap-4 md:items-center space-y-2 md:space-y-0">{
    /* Patient */
    ...
    /* Investigation */
  }<div><p className="text-sm font-medium text-slate-700 dark:text-[#cccccc]">{order.serviceName}</p>{order.billNo && <p className="text-xs font-mono text-slate-400 dark:text-[#555555] mt-0.5">{order.billNo}</p>}</div>{
    /* Referred by */
