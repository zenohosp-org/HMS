...
function RadiologyReports() {
  ...
  return <div className="space-y-5"><div className="flex items-start justify-between gap-4 flex-wrap"><div><h1 className="text-xl font-bold text-slate-900 dark:text-[#f0f0f0] flex items-center gap-2"><FileText className="w-5 h-5 text-slate-700 dark:text-slate-300" /> Radiology Reports
    ...
    /* Search */
  }<div className="bg-white dark:bg-[#111111] border border-slate-200 dark:border-[#1e1e1e] rounded-lg p-4"><div className="relative"><Search className="absolute left-3 top-1/2 -translate-y-1/2 w-4 h-4 text-slate-600" /><input
  ...
  /></div></div>{
    /* Reports table */
  }<div className="bg-white dark:bg-[#111111] border border-slate-200 dark:border-[#1e1e1e] rounded-lg overflow-hidden"><div className="px-6 py-4 border-b border-slate-100 dark:border-[#1e1e1e]"><p className="text-sm font-bold text-slate-800 dark:text-[#dddddd]">Completed Reports</p><p className="text-xs text-slate-600 dark:text-[#555555] mt-0.5">Radiology reports with findings</p></div>{loading ? <div className="flex items-center justify-center py-16"><Loader2 className="w-5 h-5 animate-spin text-slate-600" /></div> : filtered.length === 0 ? <div className="flex flex-col items-center justify-center py-16 gap-2 text-slate-600 dark:text-[#555555]"><FileText className="w-8 h-8 opacity-30" /><p className="text-sm">{search ? "No reports match your search" : "No completed reports yet"}</p></div> : <>{
    /* Column headers */
  }<div className="hidden md:grid grid-cols-[2.5fr_2fr_2fr_1.5fr_1fr_auto] gap-4 px-6 py-2.5 bg-slate-50 dark:bg-[#0d0d0d] border-b border-slate-100 dark:border-[#1e1e1e]">{["Patient", "Investigation", "Referred By", "Completed", "Priority", "Action"].map((h) => <p key={h} className="text-[10px] font-bold uppercase tracking-widest text-slate-600 dark:text-[#555555]">{h}</p>)}</div><div className="divide-y divide-slate-100 dark:divide-[#1a1a1a]">{filtered.slice((page - 1) * PAGE_SIZE, page * PAGE_SIZE).map((order) => <div key={order.id} className="px-6 py-4 hover:bg-slate-50 dark:hover:bg-[#151515] transition-colors md:grid md:grid-cols-[2.5fr_2fr_2fr_1.5fr_1fr_auto] md:gap-4 md:items-center space-y-2 md:space-y-0">{
    /* Patient */
  }<div className="flex items-center gap-2.5"><div className="w-8 h-8 rounded-full bg-slate-100 dark:bg-[#1e1e1e] flex items-center justify-center shrink-0"><User className="w-4 h-4 text-slate-500 dark:text-[#888]" /></div><div><p className="text-sm font-semibold text-slate-800 dark:text-[#dddddd] leading-tight">{order.patientName}</p><p className="text-xs text-slate-600 dark:text-[#555555]">{order.patientMrn}</p></div></div>{
    /* Investigation */
  }<div><p className="text-sm font-medium text-slate-700 dark:text-[#cccccc]">{order.serviceName}</p>{order.billNo && <p className="text-xs font-mono text-slate-600 dark:text-[#555555] mt-0.5">{order.billNo}</p>}</div>{
    /* Referred by */
    ...
    className="flex items-center gap-1.5 px-3 py-1.5 rounded-lg border border-slate-200 dark:border-[#333] text-xs font-bold text-slate-700 dark:text-slate-300 hover:bg-slate-50 dark:hover:bg-[#1a1a1a] transition-colors"
