...
import { patientApi, staffApi, hospitalServiceApi, radiologyApi, type Patient, type StaffUser, type HospitalService } from '@/utils/api'
...
export default function NewOrderModal({ onClose, onCreated }: Props) {
    ...
    useEffect(() => {
        ...
        staffApi.list(user.hospitalId)
            .then(users => setTechnicians(users.filter(u => u.role?.toLowerCase() === 'technician')))
            .catch(() => {})
    }, [user?.hospitalId])
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-900/50 backdrop-blur-sm">
            <div className="bg-white dark:bg-[#111111] rounded-2xl shadow-xl w-full max-w-lg border border-slate-200 dark:border-[#2a2a2a] flex flex-col max-h-[90vh]">
                ...
                <div className="flex items-center justify-between px-6 py-5 border-b border-slate-100 dark:border-[#1e1e1e] shrink-0">
                    <div>
                        <h2 className="text-base font-bold text-slate-900 dark:text-white">New Radiology Order</h2>
                        <p className="text-xs text-slate-500 dark:text-[#666666] mt-0.5">Create an imaging request for a patient</p>
                    </div>
                    <button onClick={onClose} className="p-2 text-slate-400 hover:text-slate-600 dark:hover:text-[#cccccc] rounded-lg hover:bg-slate-100 dark:hover:bg-[#1a1a1a] transition-colors">
                        <X className="w-4 h-4" />
                    </button>
                </div>
                ...
                <form onSubmit={handleSubmit} className="flex-1 overflow-y-auto p-6 space-y-5">
                    ...
                    <div>
                        ...
                        {selectedPatient ? (
                            <div className="flex items-center justify-between px-3 py-2.5 rounded-lg border border-emerald-200 dark:border-emerald-500/30 bg-emerald-50 dark:bg-emerald-500/10">
                                <div>
                                    <p className="text-sm font-semibold text-emerald-800 dark:text-emerald-300">
                                        {selectedPatient.firstName} {selectedPatient.lastName}
                                    </p>
                                    <p className="text-xs text-emerald-600 dark:text-emerald-500">{selectedPatient.mrn}</p>
                                </div>
                                <button type="button" onClick={() => { setSelectedPatient(null); setPatientSearch('') }} className="text-emerald-500 hover:text-emerald-700 dark:hover:text-emerald-300 transition-colors">
                                    <X className="w-4 h-4" />
                                </button>
                            </div>
                        ) : (
                            <div className="relative">
                                ...
                                {patients.length > 0 && (
                                    <div className="absolute top-full left-0 right-0 mt-1 bg-white dark:bg-[#1a1a1a] border border-slate-200 dark:border-[#333333] rounded-xl shadow-lg z-10 overflow-hidden">
                                        {patients.map(p => (
                                            <button
                                                key={p.id}
                                                type="button"
                    ...
                    <div className="grid grid-cols-2 gap-4">
                        ...
                        <div>
                            ...
                            <input className={inputCls} placeholder="e.g. DIAG-2026-0001" value={form.billNo}
