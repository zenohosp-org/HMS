...
function RadiologyReportView() {
  ...
  ><Printer className="w-4 h-4" /> Print Report
    ...
    /* Hospital header */
                            ...
                            </span>{order.reportId && <p className="text-xs text-slate-600 dark:text-[#555555] mt-2 font-mono">
    ...
    /* Signature row */
  }<div className="flex items-end justify-between mt-10 pt-6 border-t border-slate-200 dark:border-[#1e1e1e]"><div className="flex flex-col items-center gap-2"><div className="w-24 h-16 rounded-lg border-2 border-dashed border-slate-200 dark:border-[#2a2a2a] flex items-center justify-center"><p className="text-[10px] text-slate-600 dark:text-[#555555]">QR Code</p></div></div><div className="text-right"><div className="w-40 border-b border-slate-400 dark:border-[#555555] mb-2" /><p className="text-xs font-semibold text-slate-600 dark:text-[#aaaaaa]">Radiologist</p><p className="text-[10px] text-slate-600 dark:text-[#555555] mt-0.5">Signature &amp; Stamp</p></div></div></div></div></div>;
}
function InfoRow({ label, value, bold }) {
  return <div><p className="text-[10px] font-bold uppercase tracking-widest text-slate-600 dark:text-[#555555] mb-0.5">{label}</p><p className={`text-sm ${bold ? "font-bold text-slate-900 dark:text-white" : "text-slate-700 dark:text-[#cccccc]"}`}>{value ?? "N/A"}</p></div>;
}
export {
