...
export default function RadiologyReports() {
    ...
    return (
        <div className="space-y-5">
            ...
            <div className="flex items-start justify-between gap-4 flex-wrap">
                <div>
                    <h1 className="text-xl font-bold text-slate-900 dark:text-[#f0f0f0] flex items-center gap-2">
                        <FileText className="w-5 h-5 text-violet-500" /> Radiology Reports
                    </h1>
            ...
            {/* Reports table */}
            <div className="bg-white dark:bg-[#111111] border border-slate-200 dark:border-[#1e1e1e] rounded-xl overflow-hidden border-t-4 border-violet-400">
                <div className="px-6 py-4 border-b border-slate-100 dark:border-[#1e1e1e]">
                    <p className="text-sm font-bold text-slate-800 dark:text-[#dddddd]">Completed Reports</p>
                    <p className="text-xs text-slate-400 dark:text-[#555555] mt-0.5">Radiology reports with findings</p>
                </div>
                ...
                {loading ? (
                    ...
                    <div className="flex flex-col items-center justify-center py-16 gap-2 text-slate-400 dark:text-[#555555]">
                        <FileText className="w-8 h-8 opacity-30" />
                        <p className="text-sm">{search ? 'No reports match your search' : 'No completed reports yet'}</p>
                    </div>
                ) : (
                    <>
                        {/* Column headers */}
                        <div className="hidden md:grid grid-cols-[2.5fr_2fr_2fr_1.5fr_1fr_auto] gap-4 px-6 py-2.5 bg-slate-50 dark:bg-[#0d0d0d] border-b border-slate-100 dark:border-[#1e1e1e]">
                            ...
                            {filtered.slice((page - 1) * PAGE_SIZE, page * PAGE_SIZE).map(order => (
                                <div key={order.id} className="px-6 py-4 hover:bg-slate-50 dark:hover:bg-[#151515] transition-colors md:grid md:grid-cols-[2.5fr_2fr_2fr_1.5fr_1fr_auto] md:gap-4 md:items-center space-y-2 md:space-y-0">
                                    ...
                                    <div>
                                        ...
                                        {order.billNo && (
                                            <p className="text-xs font-mono text-slate-400 dark:text-[#555555] mt-0.5">{order.billNo}</p>
                                        )}
                                    </div>

                                    {/* Referred by */}
                                    <div className="flex items-center gap-1.5">
                                        <p className="text-sm text-slate-600 dark:text-[#aaaaaa]">{order.referredByName ?? '—'}</p>
                                    </div>

                                    {/* Completed */}
                                    <div className="flex items-center gap-1.5 text-xs text-slate-500 dark:text-[#888888]">
