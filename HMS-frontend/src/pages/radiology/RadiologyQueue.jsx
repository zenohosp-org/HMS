...
function RadiologyQueue() {
  ...
  return <div className="space-y-5" onClick={() => setActionMenu(null)}>{
    /* Page header */
  }<div className="flex items-start justify-between gap-4 flex-wrap"><div><h1 className="text-xl font-bold text-slate-900 dark:text-[#f0f0f0] flex items-center gap-2"><ScanLine className="w-5 h-5 text-slate-700 dark:text-slate-300" /> Radiology Queue
                    ...
                    </p></div><div className="flex items-center gap-3"><div className="flex items-center gap-2 text-xs font-semibold"><span className="flex items-center gap-1.5 px-3 py-1.5 rounded-full border border-slate-200 dark:border-[#333] bg-slate-100 dark:bg-[#1e1e1e] text-slate-600 dark:text-[#888]"><ScanLine className="w-3 h-3" /> {stats.pendingScan} awaiting scan
                        </span><span className="flex items-center gap-1.5 px-3 py-1.5 rounded-full border border-slate-200 dark:border-[#333] bg-slate-100 dark:bg-[#1e1e1e] text-slate-600 dark:text-[#888]"><Clock className="w-3 h-3" /> {stats.awaitingReport} awaiting report
                        </span><span className="flex items-center gap-1.5 px-3 py-1.5 rounded-full border border-slate-200 dark:border-[#333] bg-slate-100 dark:bg-[#1e1e1e] text-slate-600 dark:text-[#888]"><CheckCircle2 className="w-3 h-3" /> {stats.reportGenerated} done
    ...
    /* Stat cards */
  }<div className="grid grid-cols-3 gap-4"><div className="bg-white dark:bg-[#111111] border border-slate-200 dark:border-[#1e1e1e] rounded-lg p-5 flex items-center justify-between"><div><p className="text-xs font-semibold text-slate-600 dark:text-[#666666] uppercase tracking-wider">Pending Scans</p><p className="text-3xl font-bold text-amber-600 dark:text-amber-400 mt-1">{stats.pendingScan}</p></div><ScanLine className="w-8 h-8 text-amber-500" /></div><div className="bg-white dark:bg-[#111111] border border-slate-200 dark:border-[#1e1e1e] rounded-lg p-5 flex items-center justify-between"><div><p className="text-xs font-semibold text-slate-600 dark:text-[#666666] uppercase tracking-wider">Awaiting Reports</p><p className="text-3xl font-bold text-blue-600 dark:text-blue-400 mt-1">{stats.awaitingReport}</p></div><Clock className="w-8 h-8 text-blue-500" /></div><div className="bg-white dark:bg-[#111111] border border-slate-200 dark:border-[#1e1e1e] rounded-lg p-5 flex items-center justify-between"><div><p className="text-xs font-semibold text-slate-600 dark:text-[#666666] uppercase tracking-wider">Completed Today</p><p className="text-3xl font-bold text-slate-900 dark:text-white mt-1">{stats.reportGenerated}</p></div><CheckCircle2 className="w-8 h-8 text-slate-600" /></div></div>{
    /* Search + filters */
  }<div className="bg-white dark:bg-[#111111] border border-slate-200 dark:border-[#1e1e1e] rounded-lg p-4 flex flex-col sm:flex-row gap-3"><div className="relative flex-1"><Search className="absolute left-3 top-1/2 -translate-y-1/2 w-4 h-4 text-slate-600" /><input
  ...
  >{p}</button>)}</div></div>{loading ? <div className="flex items-center justify-center py-20"><Loader2 className="w-5 h-5 animate-spin text-slate-900 dark:text-white" /></div> : <>{
  ...
  />{
    ...
    color="border-slate-300 dark:border-[#333]"
    ...
    actionCls="bg-slate-900 dark:bg-white hover:bg-slate-700 dark:hover:bg-slate-100 text-white dark:text-slate-900"
...
function QueueSection({ title, subtitle, color, orders, emptyText, emptySubtext, actionLabel, actionCls, onAction, loadingId, actionMenu, setActionMenu, showScanAction }) {
  return <div className={`bg-white dark:bg-[#111111] border border-slate-200 dark:border-[#1e1e1e] rounded-lg overflow-hidden border-t-4 ${color}`}><div className="px-6 py-4 border-b border-slate-100 dark:border-[#1e1e1e]"><p className="text-sm font-bold text-slate-800 dark:text-[#dddddd]">{title}</p><p className="text-xs text-slate-600 dark:text-[#555555] mt-0.5">{subtitle}</p></div>{orders.length === 0 ? <div className="flex flex-col items-center justify-center py-14 gap-2 text-slate-600 dark:text-[#555555]"><ScanLine className="w-8 h-8 opacity-30" /><p className="text-sm font-medium">{emptyText}</p><p className="text-xs">{emptySubtext}</p></div> : <div className="divide-y divide-slate-100 dark:divide-[#1a1a1a]">{
    /* Column headers */
  }<div className="hidden md:grid grid-cols-[2.5fr_2fr_1.5fr_1fr_1fr_auto] gap-4 px-6 py-2.5 bg-slate-50 dark:bg-[#0d0d0d] border-b border-slate-100 dark:border-[#1e1e1e]">{["Patient", "Investigation", "Technician", "Priority", "Scheduled", ""].map((h) => <p key={h} className="text-[10px] font-bold uppercase tracking-widest text-slate-600 dark:text-[#555555]">{h}</p>)}</div>{orders.map((order) => {
    ...
    return <div key={order.id} className="px-6 py-4 hover:bg-slate-50 dark:hover:bg-[#151515] transition-colors md:grid md:grid-cols-[2.5fr_2fr_1.5fr_1fr_1fr_auto] md:gap-4 md:items-center space-y-2 md:space-y-0" onClick={(e) => e.stopPropagation()}>{
      /* Patient */
    }<div className="flex items-center gap-2.5"><div className="w-8 h-8 rounded-full bg-slate-100 dark:bg-[#222222] flex items-center justify-center text-xs font-bold text-slate-600 dark:text-[#888888] shrink-0">{order.patientName[0]}</div><div><p className="text-sm font-semibold text-slate-800 dark:text-[#dddddd] leading-tight">{order.patientName}</p><p className="text-xs text-slate-600 dark:text-[#555555]">{order.patientMrn}</p></div></div>{
      /* Investigation */
    }<div><p className="text-sm font-medium text-slate-700 dark:text-[#cccccc]">{order.serviceName}</p>{order.referredByName && <div className="flex items-center gap-1 mt-0.5"><Stethoscope className="w-3 h-3 text-slate-600 dark:text-[#555555]" /><p className="text-xs text-slate-600 dark:text-[#555555]">{order.referredByName}</p></div>}</div>{
      /* Technician */
    }<div>{order.technicianName ? <div className="flex items-center gap-1.5"><User className="w-3 h-3 text-slate-600 dark:text-[#555555] shrink-0" /><p className="text-xs text-slate-600 dark:text-[#aaaaaa]">{order.technicianName}</p></div> : <p className="text-xs text-slate-300 dark:text-[#444444]">Unassigned</p>}</div>{
      /* Priority */
