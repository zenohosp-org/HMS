...
export default function CreateInvoice() {
    ...
    const handleSubmit = async () => {
        ...
        try {
            ...
            loadLogs()
            window.print()
        } catch { notify('Failed to create invoice', 'error') }
    ...
    return (
            ...
            <div className="flex-1 overflow-y-auto p-6 space-y-4 min-w-0">
                ...
                <div className={cardCls}>
                    <p className="text-xs font-bold text-slate-500 dark:text-[#888888] uppercase tracking-wider mb-3 flex items-center gap-2">
                        <span className="w-4 h-4 rounded-full bg-slate-100 dark:bg-[#222222] text-[10px] font-bold text-slate-600 dark:text-[#aaaaaa] flex items-center justify-center">1</span>
                        Select Patient
                    </p>
                    {patient ? (
                        ...
                        <div className="relative">
                            ...
                            <input className={`${inputCls} pl-9`} placeholder="Search by name or MRN…" value={patientSearch} onChange={e => setPatientSearch(e.target.value)} />
                ...
                {patient && (loadingSuggestions || hasSuggestions) && (
                    <div className={cardCls}>
                        <p className="text-xs font-bold text-slate-500 dark:text-[#888888] uppercase tracking-wider mb-3 flex items-center gap-2">
                            <Sparkles className="w-3.5 h-3.5 text-amber-500" /> Detected Items
                                    ...
                                    return (
                                        <div className="flex items-center justify-between px-3 py-2.5 rounded-lg border border-orange-200 dark:border-orange-500/20 bg-orange-50 dark:bg-orange-500/5">
                                            <div className="flex items-center gap-2.5">
                                                <div className="w-7 h-7 rounded-lg bg-orange-100 dark:bg-orange-500/20 flex items-center justify-center shrink-0"><BedDouble className="w-3.5 h-3.5 text-orange-600 dark:text-orange-400" /></div>
                                                <div>
                                            ...
                                            <button onClick={() => !added && addItem({ itemType: 'ROOM_CHARGE', description: `Room ${r.roomNumber} (${r.roomType}) — ${r.daysStayed} day${r.daysStayed !== 1 ? 's' : ''}`, quantity: Number(r.daysStayed), unitPrice: r.pricePerDay, totalPrice: r.totalCharge }, key)}
                                                className={`text-xs font-bold px-2.5 py-1 rounded-lg transition-colors ${added ? 'bg-slate-100 dark:bg-[#222222] text-slate-400 cursor-default' : 'bg-orange-500 hover:bg-orange-600 text-white'}`}>{added ? '✓ Added' : '+ Add'}</button>
                                        </div>
                                    )
                                })()}
                                {suggestions.radiologyOrders.map(r => {
                                    ...
                                    return (
                                        <div key={key} className="flex items-center justify-between px-3 py-2.5 rounded-lg border border-violet-200 dark:border-violet-500/20 bg-violet-50 dark:bg-violet-500/5">
                                            <div className="flex items-center gap-2.5">
                                                <div className="w-7 h-7 rounded-lg bg-violet-100 dark:bg-violet-500/20 flex items-center justify-center shrink-0"><ScanLine className="w-3.5 h-3.5 text-violet-600 dark:text-violet-400" /></div>
                                                <div>
                                            ...
                                            <button onClick={() => !added && addItem({ itemType: 'RADIOLOGY', description: r.serviceName, quantity: 1, unitPrice: 0, totalPrice: 0 }, key)}
                                                className={`text-xs font-bold px-2.5 py-1 rounded-lg transition-colors ${added ? 'bg-slate-100 dark:bg-[#222222] text-slate-400 cursor-default' : 'bg-violet-500 hover:bg-violet-600 text-white'}`}>{added ? '✓ Added' : '+ Add'}</button>
                                        </div>
                                    )
                                })}
                                {suggestions.appointments.map(a => {
                                    ...
                                    return (
                                        <div key={key} className="flex items-center justify-between px-3 py-2.5 rounded-lg border border-blue-200 dark:border-blue-500/20 bg-blue-50 dark:bg-blue-500/5">
                                            <div className="flex items-center gap-2.5">
                                                <div className="w-7 h-7 rounded-lg bg-blue-100 dark:bg-blue-500/20 flex items-center justify-center shrink-0"><Stethoscope className="w-3.5 h-3.5 text-blue-600 dark:text-blue-400" /></div>
                                                <div>
                                            ...
                                            <button onClick={() => !added && addItem({ itemType: 'CONSULTATION', description: `Consultation — ${a.doctorName}${a.specialization ? ` (${a.specialization})` : ''}`, quantity: 1, unitPrice: a.consultationFee, totalPrice: a.consultationFee }, key)}
                                                className={`text-xs font-bold px-2.5 py-1 rounded-lg transition-colors ${added ? 'bg-slate-100 dark:bg-[#222222] text-slate-400 cursor-default' : 'bg-blue-500 hover:bg-blue-600 text-white'}`}>{added ? '✓ Added' : '+ Add'}</button>
                                        </div>
                                    )
                                })}
                            </div>
                ...
                <div className={cardCls}>
                    <p className="text-xs font-bold text-slate-500 dark:text-[#888888] uppercase tracking-wider mb-3 flex items-center gap-2">
                        <span className="w-4 h-4 rounded-full bg-slate-100 dark:bg-[#222222] text-[10px] font-bold text-slate-600 dark:text-[#aaaaaa] flex items-center justify-center">2</span>
                        Referred By <span className="font-normal normal-case text-slate-400">(Optional)</span>
                    </p>
                    <select className={inputCls} value={referredById} onChange={e => setReferredById(e.target.value)}>
                        ...
                        {doctors.map(d => <option key={d.id} value={d.id}>{d.firstName} {d.lastName}{d.specialization ? ` — ${d.specialization}` : ''}</option>)}
                    </select>
                </div>
                ...
                <div className={cardCls}>
                    <p className="text-xs font-bold text-slate-500 dark:text-[#888888] uppercase tracking-wider mb-3 flex items-center gap-2">
                        <span className="w-4 h-4 rounded-full bg-slate-100 dark:bg-[#222222] text-[10px] font-bold text-slate-600 dark:text-[#aaaaaa] flex items-center justify-center">3</span>
                        Add Tests &amp; Services
                    </p>
                    <div className="grid grid-cols-2 gap-3">
                        <div>
                            <label className="block text-xs text-slate-400 dark:text-[#666666] mb-1.5 flex items-center gap-1"><FlaskConical className="w-3 h-3 text-violet-500" /> Search Lab / Services</label>
                            <div className="relative">
                                <input className={inputCls} placeholder="Search by test name…" value={serviceSearch} onChange={e => setServiceSearch(e.target.value)} />
                                    ...
                                    <div className="absolute top-full left-0 right-0 mt-1 bg-white dark:bg-[#1a1a1a] border border-slate-200 dark:border-[#333333] rounded-xl shadow-xl z-20 overflow-hidden">
                                        {serviceResults.map(s => (
                                            <button key={s.id} type="button"
                                                onClick={() => { addItem({ itemType: 'LAB_TEST', serviceId: s.id, description: s.name, quantity: 1, unitPrice: s.price, totalPrice: s.price }); setServiceSearch(''); setServiceResults([]) }}
                        ...
                        <div>
                            <label className="block text-xs text-slate-400 dark:text-[#666666] mb-1.5 flex items-center gap-1"><Pill className="w-3 h-3 text-emerald-500" /> Add Medicine</label>
                            <button onClick={() => addItem({ itemType: 'MEDICINE', description: '', quantity: 1, unitPrice: 0, totalPrice: 0 })}
                                className="w-full px-3 py-2 rounded-xl border border-dashed border-emerald-300 dark:border-emerald-500/30 text-sm text-emerald-600 dark:text-emerald-400 hover:bg-emerald-50 dark:hover:bg-emerald-500/10 transition-colors text-left">
                ...
                <div className={cardCls}>
                    <div className="flex items-center justify-between mb-3">
                        <p className="text-xs font-bold text-slate-500 dark:text-[#888888] uppercase tracking-wider flex items-center gap-2">
                            <span className="w-4 h-4 rounded-full bg-slate-100 dark:bg-[#222222] text-[10px] font-bold text-slate-600 dark:text-[#aaaaaa] flex items-center justify-center">4</span>
                            Invoice Items
                        </p>
                                    ...
                                    <div key={item.key} className="grid grid-cols-12 gap-2 items-center py-2 group px-1">
                                        <div className="col-span-1">
                                            <select className="w-full text-[10px] rounded border border-slate-100 dark:border-[#2a2a2a] bg-slate-50 dark:bg-[#1a1a1a] px-1 py-1 text-slate-700 dark:text-[#cccccc] focus:outline-none"
                                                value={item.itemType ?? 'CUSTOM'} onChange={e => updateItem(item.key, { itemType: e.target.value as ItemType })}>
                                                {(Object.keys(TYPE_META) as ItemType[]).map(t => <option key={t} value={t}>{TYPE_META[t].label}</option>)}
                                            </select>
                                        </div>
                ...
                <button onClick={handleSubmit} disabled={saving || !patient || items.length === 0}
                    className="w-full py-3.5 rounded-2xl bg-emerald-500 hover:bg-emerald-600 disabled:opacity-50 text-white font-bold flex items-center justify-center gap-2 transition-colors">
        ...
        <div className="hidden print:block bg-white text-black p-8">
            <div className="flex justify-between items-start mb-6">
                <div><h1 className="text-2xl font-bold">{user?.hospitalName}</h1><p className="text-sm text-gray-500 mt-1">Tax Invoice</p></div>
                <div className="text-right text-sm"><p className="font-bold text-lg">#{invoiceNo}</p><p className="text-gray-500">{new Date().toLocaleDateString('en-IN')}</p></div>
            </div>
            ...
            <table className="w-full text-sm border-collapse">
                <thead><tr className="border-b-2 border-black"><th className="text-left py-2">Description</th><th className="text-center py-2 w-16">Qty</th><th className="text-right py-2 w-24">Unit Price</th><th className="text-right py-2 w-24">Total</th></tr></thead>
                <tbody>{items.map((i, idx) => <tr key={idx} className="border-b border-gray-100"><td className="py-1.5">{i.description}</td><td className="text-center py-1.5">{i.quantity}</td><td className="text-right py-1.5">{fmt(i.unitPrice)}</td><td className="text-right py-1.5">{fmt(i.totalPrice)}</td></tr>)}</tbody>
            </table>
