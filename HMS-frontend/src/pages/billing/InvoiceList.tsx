...
import { invoiceApi, type Invoice } from '@/utils/api'
                            ...
                            {isLoading ? (
                                Array(5).fill(0).map((_, i) => (
                                    <tr key={i} className="animate-pulse">
                                        <td colSpan={5} className="px-6 py-4 h-16 bg-slate-50/20 dark:bg-white/5" />
                                    </tr>
                                ))
                                ...
                                <tr key={inv.id} className="group hover:bg-slate-50 dark:hover:bg-white/5 transition-colors">
                                    <td className="px-6 py-4">
                                        ...
                                        <div className="text-[10px] text-slate-500 uppercase tracking-wider font-bold mt-0.5">ID: {inv.id?.slice(0,8)}...</div>
                                    </td>
                                    <td className="px-6 py-4 text-sm text-slate-600 dark:text-[#aaaaaa]">
                                        {new Date(inv.createdAt!).toLocaleDateString()}
                                    </td>
                                    <td className="px-6 py-4">
                                        <div className="font-bold text-slate-900 dark:text-white">₹{inv.total.toLocaleString()}</div>
                                        <div className="text-[10px] text-emerald-500 font-bold uppercase tracking-wider">Incl. Tax</div>
                                    </td>
                                    <td className="px-6 py-4">
