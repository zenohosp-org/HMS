...
function CreateInvoice() {
  ...
  >{paneOpen ? <PanelRightClose className="w-4 h-4" /> : <PanelRightOpen className="w-4 h-4" />}</button></div>{
    ...
    /* 1. Select Patient */
                        ...
                        Select Patient
                    </p>{patient ? <div className="flex items-center justify-between px-3 py-2.5 rounded-xl border border-emerald-200 dark:border-emerald-500/30 bg-emerald-50 dark:bg-emerald-500/10"><div className="flex items-center gap-2.5"><div className="w-8 h-8 rounded-full bg-emerald-100 dark:bg-emerald-500/20 flex items-center justify-center shrink-0"><User className="w-4 h-4 text-emerald-600 dark:text-emerald-400" /></div><div><p className="text-sm font-bold text-emerald-800 dark:text-emerald-300">{patient.firstName} {patient.lastName}</p><p className="text-xs text-emerald-600 dark:text-emerald-500">{patient.mrn}{patient.phone ? ` \xB7 ${patient.phone}` : ""}</p></div></div><button onClick={clearPatient} className="p-1 text-emerald-500 hover:text-emerald-700 transition-colors"><X className="w-3.5 h-3.5" /></button></div> : <div className="relative"><Search className="absolute left-3 top-1/2 -translate-y-1/2 w-3.5 h-3.5 text-slate-400" /><input className={`${inputCls} pl-9`} placeholder="Search by name or MRN…" value={patientSearch} onChange={(e) => setPatientSearch(e.target.value)} />{searching && <Loader2 className="absolute right-3 top-1/2 -translate-y-1/2 w-3.5 h-3.5 animate-spin text-slate-400" />}{patientResults.length > 0 && <div className="absolute top-full left-0 right-0 mt-1 bg-white dark:bg-[#1a1a1a] border border-slate-200 dark:border-[#333333] rounded-xl shadow-xl z-30 overflow-hidden">{patientResults.map((p) => <button
    key={p.id}
    type="button"
  ...
  ><p className="text-sm font-semibold text-slate-800 dark:text-[#dddddd]">{p.firstName} {p.lastName}</p><p className="text-xs text-slate-400">{p.mrn}</p></button>)}</div>}</div>}</div>{
    /* Smart Suggestions */
  ...
  /><span className="text-red-500 font-semibold">-{fmt(discountAmt)}</span></div></div>{medicineTotal > 0 && <div className="flex justify-between text-sm text-slate-500 dark:text-[#888888]"><span>GST Medicines (18%):</span><span className="font-semibold">{fmt(gstOnMedicines)}</span></div>}<div className="flex justify-between text-base font-bold text-slate-900 dark:text-white pt-2 border-t border-slate-100 dark:border-[#1e1e1e]"><span>Grand Total:</span><span className="text-blue-600 dark:text-blue-400">{fmt(grandTotal)}</span></div></div></div></>}</div>{
    ...
    /* Bank account cards */
  }{bankAccounts.length > 0 && <div><label className="block text-xs text-slate-400 dark:text-[#666666] mb-2 flex items-center gap-1.5"><Landmark className="w-3.5 h-3.5" /> Credit payment to
    ...
    return <button
      ...
      className={`text-left p-3 rounded-xl border-2 transition-all ${isSelected ? "border-emerald-500 bg-emerald-50 dark:bg-emerald-500/10" : "border-slate-200 dark:border-[#2a2a2a] hover:border-slate-300 dark:hover:border-[#3a3a3a] bg-white dark:bg-[#1a1a1a]"}`}
