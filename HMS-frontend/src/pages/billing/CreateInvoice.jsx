...
const STATUS_CFG = {
  PAID: { label: "Paid", icon: CheckCircle2, cls: "text-slate-900 dark:text-white dark:text-slate-300 bg-slate-100 dark:bg-[#1e1e1e] dark:bg-slate-500/10 border-emerald-200 dark:border-slate-900 dark:border-white/20" },
...
function CreateInvoice() {
  ...
  ><Plus className="w-3 h-3" /> Add Custom Item
                        </button></div>{items.length === 0 ? <div className="py-8 text-center text-sm text-slate-600 dark:text-[#555555] border-2 border-dashed border-slate-100 dark:border-[#1e1e1e] rounded-lg">
                            No items yet — detect from patient or add manually
                        </div> : <><div className="grid grid-cols-12 gap-2 pb-2 border-b border-slate-100 dark:border-[#1e1e1e] text-[10px] font-bold uppercase tracking-wider text-slate-600 dark:text-[#555555] px-1"><div className="col-span-1">Type</div><div className="col-span-5">Description</div><div className="col-span-2 text-center">Qty</div><div className="col-span-2 text-right">Unit ₹</div><div className="col-span-2 text-right">Total ₹</div></div><div className="divide-y divide-slate-50 dark:divide-[#1a1a1a]">{items.map((item) => <div key={item.key} className="grid grid-cols-12 gap-2 items-center py-2 group px-1"><div className="col-span-1"><select
  ...
  ><p className="text-xs font-bold text-slate-600 dark:text-[#666666]">No account</p><p className="text-[11px] text-slate-300 dark:text-[#444444] mt-0.5">Skip bank credit</p></button></div>{selectedAccount && <p className="text-xs text-slate-600 dark:text-[#666666] mt-2">
