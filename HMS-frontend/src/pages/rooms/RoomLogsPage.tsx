...
import { roomLogsApi, type RoomLog } from '@/utils/api'
import {
    ArrowLeft, Search, Loader2, Bed, User, Users,
    PlusCircle, LogOut, UserCheck, UserCog, CalendarClock,
} from 'lucide-react'

const EVENT_META: Record<RoomLog['event'], { label: string; cls: string; icon: React.ElementType }> = {
...
export default function RoomLogsPage() {
    ...
    const fetchLogs = useCallback(async () => {
        ...
        try {
            const data = roomId
                ...
                : await roomLogsApi.getHospitalLogs(user.hospitalId, debouncedSearch || undefined)
            setLogs(data)
        } catch {
            setLogs([])
        } finally {
    ...
    const filteredLogs = roomId && debouncedSearch
        ? logs.filter(l => {
            const s = debouncedSearch.toLowerCase()
            return (l.roomNumber?.toLowerCase().includes(s)) ||
                   (l.patientName?.toLowerCase().includes(s)) ||
                   (l.patientMrn?.toLowerCase().includes(s)) ||
    ...
    return (
        <div className="space-y-5">
            ...
            <div className="flex items-center gap-4">
                ...
                <div>
                    <h1 className="text-xl font-bold text-slate-900 dark:text-[#f0f0f0]">
                        {roomNumber ? `Room ${roomNumber} — Activity Log` : 'Room Activity Log'}
                    </h1>
                    <p className="text-sm text-slate-500 dark:text-[#666666] mt-0.5">
                        {loading ? 'Loading…' : `${filteredLogs.length} event${filteredLogs.length !== 1 ? 's' : ''}`}
                    </p>
                </div>
            </div>
            ...
            <div className="bg-white dark:bg-[#111111] border border-slate-200 dark:border-[#1e1e1e] rounded-xl overflow-hidden">
                ...
                {/* Column headers */}
                <div className="hidden md:grid grid-cols-[2fr_2fr_2fr_1.5fr_1fr] gap-4 px-6 py-3 border-b border-slate-100 dark:border-[#1e1e1e] bg-slate-50 dark:bg-[#0d0d0d]">
                    <p className="text-[10px] font-bold uppercase tracking-widest text-slate-400 dark:text-[#555555]">Event</p>
                    <p className="text-[10px] font-bold uppercase tracking-widest text-slate-400 dark:text-[#555555]">Patient</p>
                    <p className="text-[10px] font-bold uppercase tracking-widest text-slate-400 dark:text-[#555555]">Attender</p>
                    <p className="text-[10px] font-bold uppercase tracking-widest text-slate-400 dark:text-[#555555]">Performed By</p>
                    <p className="text-[10px] font-bold uppercase tracking-widest text-slate-400 dark:text-[#555555] text-right">Time</p>
                </div>
                            ...
                            return (
                                ...
                                >
                                    ...
                                    <div className="flex items-center gap-2 flex-wrap">
                                        <div className="w-7 h-7 rounded-lg border border-slate-200 dark:border-[#2a2a2a] bg-white dark:bg-[#1a1a1a] flex items-center justify-center shrink-0">
                                            <Icon className="w-3.5 h-3.5 text-slate-500 dark:text-[#888888]" />
                                        </div>
                                        <div>
                                            <span className={`text-[10px] font-bold uppercase tracking-wide px-2 py-0.5 rounded-full border ${meta.cls}`}>
                                                {meta.label}
                                            </span>
                                            <p className="text-xs font-bold text-slate-700 dark:text-[#cccccc] mt-1">
                                                {log.roomNumber}
                                                {log.allocationToken && (
                                                    <span className="ml-2 font-mono text-violet-600 dark:text-violet-400 font-semibold">
                                                        #{log.allocationToken}
                                                    </span>
                                                )}
                                            </p>
                                    ...
                                    <div>
                                        {log.patientName ? (
                                            <div className="flex items-start gap-1.5">
                                                <User className="w-3.5 h-3.5 text-slate-400 dark:text-[#555555] shrink-0 mt-0.5" />
                                                <div>
                                                    <p className="text-sm font-semibold text-slate-800 dark:text-[#dddddd] leading-tight">{log.patientName}</p>
                                                    {log.patientMrn && <p className="text-xs text-slate-400 dark:text-[#555555]">{log.patientMrn}</p>}
                                                </div>
                                            </div>
                                        ) : (
                                    ...
                                    <div>
                                        {log.attenderName ? (
                                            <div className="flex items-center gap-1.5">
                                                <Users className="w-3.5 h-3.5 text-slate-400 dark:text-[#555555] shrink-0" />
                                                <p className="text-sm text-slate-700 dark:text-[#cccccc]">{log.attenderName}</p>
                                            </div>
                                        ) : (
                                            <p className="text-xs text-slate-300 dark:text-[#444444]">—</p>
                                        )}
                                    ...
                                    <div>
                                        {log.performedBy ? (
                                            <p className="text-sm font-medium text-slate-700 dark:text-[#cccccc]">{log.performedBy}</p>
                                        ) : (
                                            <p className="text-xs text-slate-300 dark:text-[#444444]">—</p>
                                        )}
                                    </div>
                                    ...
                                    <div className="md:text-right">
                                        <p className="text-xs font-medium text-slate-500 dark:text-[#888888]" title={formatFull(log.createdAt)}>
                                            {formatRelative(log.createdAt)}
                                        </p>
                                        <p className="text-[10px] text-slate-400 dark:text-[#555555] mt-0.5">{formatFull(log.createdAt)}</p>
                                    </div>
                                </div>
                            )
                        })}
