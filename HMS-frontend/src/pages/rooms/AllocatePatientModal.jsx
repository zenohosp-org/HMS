...
function AllocatePatientModal({ roomId, roomNumber, onClose, onSuccess }) {
  ...
  return <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-900/50 backdrop-blur-sm"><div className="bg-white dark:bg-[#111111] rounded-2xl shadow-xl w-full max-w-2xl max-h-[90vh] flex flex-col overflow-hidden border border-slate-200 dark:border-[#2a2a2a]"><div className="flex items-center justify-between p-5 border-b border-slate-100 dark:border-[#222222] shrink-0"><h2 className="text-lg font-bold text-slate-900 dark:text-white flex items-center gap-2">
                        Allocate Patient <span className="text-slate-400">to Room {roomNumber}</span></h2><button onClick={onClose} className="p-2 text-slate-400 hover:text-slate-600 dark:hover:text-[#aaaaaa] rounded-lg hover:bg-slate-100 dark:hover:bg-[#1a1a1a] transition-colors"><X className="w-5 h-5" /></button></div><form onSubmit={handleSubmit} className="flex-1 overflow-y-auto p-5 space-y-5">{
    /* Patient Search */
  ...
  /></div><div className="h-44 overflow-y-auto space-y-2 border border-slate-100 dark:border-[#222222] rounded-xl p-2 bg-slate-50 dark:bg-[#0d0d0d]">{loading ? <div className="h-full flex items-center justify-center text-slate-400"><Loader2 className="w-5 h-5 animate-spin" /></div> : filteredPatients.length === 0 ? <div className="h-full flex items-center justify-center text-sm text-slate-400">No patients found.</div> : filteredPatients.map((p) => <div
    key={p.id}
