...
function AllocatePatientModal({ roomId, roomNumber, onClose, onSuccess }) {
  ...
  useEffect(() => {
    ...
    setLoading(true);
    api.get(`/patients?hospitalId=${user.hospitalId}`).then((r) => setPatients(r.data)).catch(() => {
  ...
  return <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-900/50 backdrop-blur-sm"><div className="bg-white dark:bg-[#111111] rounded-lg shadow-xl w-full max-w-2xl max-h-[90vh] flex flex-col overflow-hidden border border-slate-200 dark:border-[#2a2a2a]"><div className="flex items-center justify-between p-5 border-b border-slate-100 dark:border-[#222222] shrink-0"><h2 className="text-lg font-bold text-slate-900 dark:text-white flex items-center gap-2">
                        Allocate Patient <span className="text-slate-600">to Room {roomNumber}</span></h2><button onClick={onClose} className="p-2 text-slate-600 hover:text-slate-600 dark:hover:text-[#aaaaaa] rounded-lg hover:bg-slate-100 dark:hover:bg-[#1a1a1a] transition-colors"><X className="w-5 h-5" /></button></div><form onSubmit={handleSubmit} className="flex-1 overflow-y-auto p-5 space-y-5">{
    /* Patient Search */
  ...
  /></div><div className="h-44 overflow-y-auto space-y-2 border border-slate-100 dark:border-[#222222] rounded-lg p-2 bg-slate-50 dark:bg-[#0d0d0d]">{loading ? <div className="h-full flex items-center justify-center text-slate-600"><Loader2 className="w-5 h-5 animate-spin" /></div> : filteredPatients.length === 0 ? <div className="h-full flex items-center justify-center text-sm text-slate-600">No patients found.</div> : filteredPatients.map((p) => <div
    key={p.id}
  ...
  ><div><p className={`text-sm font-bold ${selectedPatientId === p.id ? "text-emerald-900 dark:text-slate-500" : "text-slate-800 dark:text-white"}`}>{p.firstName} {p.lastName}</p><p className="text-xs text-slate-500 dark:text-[#666666] mt-0.5">{p.mrn}</p></div><div className={`w-4 h-4 rounded-full border flex items-center justify-center shrink-0
    ...
    /* Attender Details */
                            ...
                            Patient Attender
                        </h3><div className="grid grid-cols-2 gap-3"><div className="col-span-2"><label className={labelCls}>Attender Name</label><input
    type="text"
    className={inputCls}
    placeholder="Full name of attender"
    value={attender.name}
    onChange={(e) => setAttender({ ...attender, name: e.target.value })}
  /></div><div><label className={labelCls}>Phone</label><input
    type="text"
    className={inputCls}
  ...
  ><option value="">Select</option><option value="Spouse">Spouse</option><option value="Parent">Parent</option><option value="Child">Child</option><option value="Sibling">Sibling</option><option value="Friend">Friend</option></select></div></div></div>{
    /* Discharge Time */
  }<div><label className={labelCls}>Estimated Discharge Time (Optional)</label><input
    type="datetime-local"
    className={inputCls}
    value={approxDischarge}
    onChange={(e) => setApproxDischarge(e.target.value)}
  /></div><div className="pt-2 border-t border-slate-100 dark:border-[#222222] flex justify-end gap-3"><button type="button" onClick={onClose} className="btn-secondary">Cancel</button><button type="submit" disabled={submitting || !selectedPatientId} className="btn-primary">{submitting ? <Loader2 className="w-4 h-4 animate-spin" /> : "Allocate Patient"}</button></div></form></div></div>;
}
export {
  AllocatePatientModal as default
};
