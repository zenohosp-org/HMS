...
function RoomActionMenu({ room, onAllocate, onAssignAttender, onDeallocate }) {
  ...
  return <div ref={ref} className="relative shrink-0"><button
    ...
    className="p-2 rounded-lg text-slate-600 hover:text-slate-600 dark:hover:text-[#cccccc] hover:bg-slate-100 dark:hover:bg-[#1a1a1a] transition-colors"
...
function Rooms() {
  ...
  ><ScrollText className="w-4 h-4" /> Logs
      ...
      /* Filters */
    }<div className="flex items-center gap-2 flex-wrap bg-white dark:bg-[#111111] border border-slate-200 dark:border-[#1e1e1e] rounded-lg px-3 py-4"><div className="flex gap-2">{["ALL", "AVAILABLE", "OCCUPIED"].map((f) => <button
      key={f}
      onClick={() => setFilter(f)}
    ...
    >{f}</button>)}</div><div className="relative flex-1"><Search className="absolute left-3 top-1/2 -translate-y-1/2 w-4 h-4 text-slate-600" /><input
      className="w-full pl-9 pr-4 py-2 rounded-lg border border-slate-300 dark:border-[#2a2a2a]
    ...
    ><div className="flex items-center gap-4"><div className={`w-12 h-12 rounded-lg flex items-center justify-center shrink-0 border
                                        ${room.status === "AVAILABLE" ? "bg-slate-100 dark:bg-[#1e1e1e] border-slate-200 dark:border-[#2a2a2a] text-slate-900 dark:text-white" : "bg-slate-50 border-slate-100 text-slate-500 dark:bg-[#1a1a1a] dark:border-[#2a2a2a] dark:text-[#888888]"}`}><Bed className="w-6 h-6" /></div><div><div className="flex items-center gap-2"><p className="text-base font-bold text-slate-900 dark:text-white leading-tight">{room.roomNumber}</p><span className={`text-[10px] font-bold uppercase tracking-wide px-2 py-0.5 rounded-full border
                                                ${room.roomType === "ICU" ? "bg-red-50 text-red-700 border-red-200 dark:bg-red-500/10 dark:text-red-400 dark:border-red-500/20" : "bg-slate-100 text-slate-600 border-slate-200 dark:bg-[#222222] dark:text-[#888888] dark:border-[#333333]"}`}>{room.roomType}</span></div><p className={`text-xs mt-1 font-medium ${room.status === "AVAILABLE" ? "text-emerald-600 dark:text-emerald-400" : "text-rose-600 dark:text-rose-400"}`}>{room.status}</p></div></div>{room.status === "OCCUPIED" && room.currentPatient ? <div className="flex-1 sm:pl-8 border-t sm:border-t-0 sm:border-l border-slate-100 dark:border-[#222222] pt-4 sm:pt-0"><div className="flex items-start justify-between gap-4"><div className="space-y-1.5"><div><p className="text-xs text-slate-500 dark:text-[#666666]">Patient</p><p className="text-sm font-bold text-slate-800 dark:text-[#dddddd]">{room.currentPatient.firstName} {room.currentPatient.lastName}</p><p className="text-[11px] text-slate-600 dark:text-[#555555]">{room.currentPatient.mrn}</p></div>{room.attenderName ? <div><p className="text-xs text-slate-500 dark:text-[#666666]">Attender</p><p className="text-sm font-medium text-slate-700 dark:text-[#cccccc]">{room.attenderName}{room.attenderRelationship && <span className="text-xs text-slate-600 dark:text-[#555555] ml-1">({room.attenderRelationship})</span>}</p>{room.attenderPhone && <p className="text-[11px] text-slate-600 dark:text-[#555555]">{room.attenderPhone}</p>}</div> : <p className="text-xs text-amber-500 dark:text-amber-400">No attender assigned</p>}</div><div className="text-right shrink-0 space-y-2">{room.allocationToken && <div><p className="text-xs text-slate-500 dark:text-[#666666] mb-0.5">Token</p><span className="inline-block px-2.5 py-1 rounded-lg bg-violet-50 dark:bg-violet-500/10 border border-violet-200 dark:border-violet-500/20 text-sm font-bold tracking-widest text-violet-700 dark:text-violet-400 font-mono">{room.allocationToken}</span></div>}{room.approxDischargeTime && <div className="hidden md:block"><div className="flex items-center gap-1.5 text-xs text-slate-500 dark:text-[#666666] mb-0.5 justify-end"><CalendarClock className="w-3.5 h-3.5" /> Est. Discharge
      </div><p className="text-xs font-medium text-slate-700 dark:text-[#aaaaaa]">{formatDateTime(room.approxDischargeTime)}</p></div>}</div></div></div> : <div className="flex-1 sm:pl-8 border-t sm:border-t-0 sm:border-l border-slate-100 dark:border-[#222222] pt-4 sm:pt-0 flex items-center"><p className="text-sm text-slate-600 dark:text-[#555555]">Ready for allocation</p></div>}<RoomActionMenu
        room={room}
