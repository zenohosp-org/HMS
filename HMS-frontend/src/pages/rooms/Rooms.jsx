...
function Rooms() {
  ...
  ><ScrollText className="w-4 h-4" /> Logs
      ...
      /* Filters */
    }<div className="flex items-center gap-2 bg-white dark:bg-[#111111] border border-slate-200 dark:border-[#1e1e1e] rounded-lg px-3 py-4"><div className="flex gap-1 shrink-0">{["ALL", "AVAILABLE", "OCCUPIED"].map((f) => <button
      key={f}
      onClick={() => setFilter(f)}
      className={`px-3 py-1 rounded-md text-xs font-semibold transition-all
                                ${filter === f ? "bg-slate-900 text-white dark:bg-white dark:text-slate-900" : "text-slate-500 dark:text-[#888] hover:text-slate-700 dark:hover:text-white"}`}
    >{f}</button>)}</div><div className="relative flex-1"><Search className="absolute left-3 top-1/2 -translate-y-1/2 w-4 h-4 text-slate-400" /><input
      className="w-full pl-9 pr-4 py-1.5 rounded-lg border border-slate-300 dark:border-[#2a2a2a]
                            bg-slate-50 dark:bg-[#1a1a1a] text-slate-900 dark:text-[#cccccc] text-sm focus:outline-none focus:ring-2 focus:ring-slate-200 dark:focus:ring-[#333]"
