...
function Rooms() {
  ...
  ><ScrollText className="w-4 h-4" /> Logs
    ...
    /* Metrics */
  }<div className="grid grid-cols-1 sm:grid-cols-3 gap-4"><div className="bg-white dark:bg-[#111111] border border-slate-200 dark:border-[#1e1e1e] rounded-xl p-4 flex items-center justify-between"><div><p className="text-sm font-semibold text-slate-500 dark:text-[#666666]">Total Rooms</p><p className="text-2xl font-bold text-slate-800 dark:text-[#e0e0e0] mt-1">{rooms.length}</p></div><div className="w-10 h-10 rounded-full bg-slate-50 dark:bg-[#1a1a1a] flex items-center justify-center"><Bed className="w-5 h-5 text-slate-500" /></div></div><div className="bg-emerald-50 dark:bg-emerald-500/10 border border-emerald-200 dark:border-emerald-500/20 rounded-xl p-4 flex items-center justify-between"><div><p className="text-sm font-semibold text-emerald-700 dark:text-emerald-400">Available</p><p className="text-2xl font-bold text-emerald-800 dark:text-emerald-300 mt-1">{availableCount}</p></div><div className="w-10 h-10 rounded-full bg-emerald-100 dark:bg-emerald-500/20 flex items-center justify-center"><Bed className="w-5 h-5 text-emerald-600 dark:text-emerald-400" /></div></div><div className="bg-blue-50 dark:bg-blue-500/10 border border-blue-200 dark:border-blue-500/20 rounded-xl p-4 flex items-center justify-between"><div><p className="text-sm font-semibold text-blue-700 dark:text-blue-400">Occupied</p><p className="text-2xl font-bold text-blue-800 dark:text-blue-300 mt-1">{occupiedCount}</p></div><div className="w-10 h-10 rounded-full bg-blue-100 dark:bg-blue-500/20 flex items-center justify-center"><Bed className="w-5 h-5 text-blue-600 dark:text-blue-400" /></div></div></div>{
    /* Filters */
  ...
  /></div></div>{
    ...
    /* Room List */
  }<div className="flex-1 min-w-0 space-y-3">{loading ? <div className="bg-white dark:bg-[#111111] border border-slate-200 dark:border-[#1e1e1e] rounded-xl p-8 text-center"><p className="text-slate-500 dark:text-[#666666]">Loading rooms…</p></div> : filteredRooms.length === 0 ? <div className="bg-white dark:bg-[#111111] border border-slate-200 dark:border-[#1e1e1e] rounded-xl p-8 text-center"><p className="text-slate-500 dark:text-[#666666]">No rooms found matching criteria.</p></div> : filteredRooms.map((room) => <div
    key={room.id}
    ...
    className={`bg-white dark:bg-[#111111] border rounded-xl p-4 flex flex-col sm:flex-row sm:items-center justify-between gap-4 cursor-pointer transition-colors
                                    ${selectedRoom?.id === room.id ? "border-slate-400 dark:border-[#444444]" : room.status === "AVAILABLE" ? "border-emerald-200 dark:border-emerald-900/40 hover:border-emerald-300 dark:hover:border-emerald-800/50" : "border-slate-200 dark:border-[#1e1e1e] hover:border-slate-300 dark:hover:border-[#2a2a2a]"}`}
  ><div className="flex items-center gap-4"><div className={`w-12 h-12 rounded-xl flex items-center justify-center shrink-0 border
                                        ${room.status === "AVAILABLE" ? "bg-emerald-50 border-emerald-100 text-emerald-600 dark:bg-emerald-500/10 dark:border-emerald-500/20 dark:text-emerald-400" : "bg-slate-50 border-slate-100 text-slate-500 dark:bg-[#1a1a1a] dark:border-[#2a2a2a] dark:text-[#888888]"}`}><Bed className="w-6 h-6" /></div><div><div className="flex items-center gap-2"><p className="text-base font-bold text-slate-900 dark:text-white leading-tight">{room.roomNumber}</p>{room.roomCode && <span className="text-[10px] font-mono font-bold text-slate-400 dark:text-[#555]">{room.roomCode}</span>}<span className={`text-[10px] font-bold uppercase tracking-wide px-2 py-0.5 rounded-full border
                                                ${room.roomType === "ICU" ? "bg-red-50 text-red-700 border-red-200 dark:bg-red-500/10 dark:text-red-400 dark:border-red-500/20" : "bg-slate-100 text-slate-600 border-slate-200 dark:bg-[#222222] dark:text-[#888888] dark:border-[#333333]"}`}>{room.roomType}</span></div><p className={`text-xs mt-1 font-medium ${room.status === "AVAILABLE" ? "text-emerald-600 dark:text-emerald-400" : "text-blue-600 dark:text-blue-400"}`}>{room.status}</p></div></div>{room.status === "OCCUPIED" && room.currentPatient ? <div className="flex-1 sm:pl-8 border-t sm:border-t-0 sm:border-l border-slate-100 dark:border-[#222222] pt-4 sm:pt-0"><div className="flex items-start justify-between gap-4"><div className="space-y-1.5"><div><p className="text-xs text-slate-500 dark:text-[#666666]">Patient</p><p className="text-sm font-bold text-slate-800 dark:text-[#dddddd]">{room.currentPatient.firstName} {room.currentPatient.lastName}</p><p className="text-[11px] text-slate-400 dark:text-[#555555]">{room.currentPatient.mrn}</p></div>{room.attenderName ? <div><p className="text-xs text-slate-500 dark:text-[#666666]">Attender</p><p className="text-sm font-medium text-slate-700 dark:text-[#cccccc]">{room.attenderName}{room.attenderRelationship && <span className="text-xs text-slate-400 dark:text-[#555555] ml-1">({room.attenderRelationship})</span>}</p>{room.attenderPhone && <p className="text-[11px] text-slate-400 dark:text-[#555555]">{room.attenderPhone}</p>}</div> : <p className="text-xs text-amber-500 dark:text-amber-400">No attender assigned</p>}</div><div className="text-right shrink-0 space-y-2">{room.allocationToken && <div><p className="text-xs text-slate-500 dark:text-[#666666] mb-0.5">Token</p><span className="inline-block px-2.5 py-1 rounded-lg bg-violet-50 dark:bg-violet-500/10 border border-violet-200 dark:border-violet-500/20 text-sm font-bold tracking-widest text-violet-700 dark:text-violet-400 font-mono">{room.allocationToken}</span></div>}{room.approxDischargeTime && <div className="hidden md:block"><div className="flex items-center gap-1.5 text-xs text-slate-500 dark:text-[#666666] mb-0.5 justify-end"><CalendarClock className="w-3.5 h-3.5" /> Est. Discharge
