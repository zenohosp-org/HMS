...
function RoomLogsPage() {
  ...
  /></div></div>{
    ...
    >{
      ...
      /* Patient */
    }<div>{log.patientName ? <div className="flex items-start gap-1.5"><User className="w-3.5 h-3.5 text-slate-400 dark:text-[#555555] shrink-0 mt-0.5" /><div><p className="text-sm font-semibold text-slate-800 dark:text-[#dddddd] leading-tight">{log.patientName}</p>{log.patientMrn && <p className="text-xs text-slate-400 dark:text-[#555555]">{log.patientMrn}</p>}</div></div> : <p className="text-xs text-slate-300 dark:text-[#444444]">—</p>}</div>{
      /* Attender */
    }<div>{log.attenderName ? <div className="flex items-center gap-1.5"><Users className="w-3.5 h-3.5 text-slate-400 dark:text-[#555555] shrink-0" /><p className="text-sm text-slate-700 dark:text-[#cccccc]">{log.attenderName}</p></div> : <p className="text-xs text-slate-300 dark:text-[#444444]">—</p>}</div>{
      /* Performed by */
      ...
      /* Time */
    }<div className="md:text-right"><p className="text-xs font-medium text-slate-500 dark:text-[#888888]" title={formatFull(log.createdAt)}>{formatRelative(log.createdAt)}</p><p className="text-[10px] text-slate-400 dark:text-[#555555] mt-0.5">{formatFull(log.createdAt)}</p></div></div>;
  })}</div>}</div></div>;
}
export {
