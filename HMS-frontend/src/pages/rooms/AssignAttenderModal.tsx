...
interface AssignAttenderModalProps {
    roomId: number
    roomNumber: string
    existing: { name?: string; phone?: string; relationship?: string }
    onClose: () => void
    onSuccess: () => void
}
...
export default function AssignAttenderModal({ roomId, roomNumber, existing, onClose, onSuccess }: AssignAttenderModalProps) {
    const { user } = useAuth()
            ...
            <div className="bg-white dark:bg-[#111111] rounded-2xl shadow-xl w-full max-w-md overflow-hidden border border-slate-200 dark:border-[#2a2a2a]">
                <div className="flex items-center justify-between p-5 border-b border-slate-100 dark:border-[#222222]">
                    <h2 className="text-lg font-bold text-slate-900 dark:text-white">
                        Assign Attender <span className="text-slate-400 font-normal">· Room {roomNumber}</span>
                    </h2>
                ...
                <form onSubmit={handleSubmit} className="p-5 space-y-4">
                    ...
                    <div className="pt-2 border-t border-slate-100 dark:border-[#222222] flex justify-end gap-3">
                        ...
                        <button type="submit" disabled={submitting} className="btn-primary">
                            {submitting ? <Loader2 className="w-4 h-4 animate-spin" /> : 'Save Attender'}
                        </button>
                    </div>
                </form>
            </div>
