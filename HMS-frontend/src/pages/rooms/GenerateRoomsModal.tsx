...
export default function GenerateRoomsModal({ onClose, onSuccess }: GenerateRoomsModalProps) {
    const { user } = useAuth()
    ...
    return (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-900/50 backdrop-blur-sm">
            <div className="bg-white dark:bg-[#111111] rounded-2xl shadow-xl w-full max-w-md overflow-hidden border border-slate-200 dark:border-[#2a2a2a]">
                <div className="flex items-center justify-between p-5 border-b border-slate-100 dark:border-[#222222]">
                    <h2 className="text-lg font-bold text-slate-900 dark:text-white">Generate Rooms</h2>
                ...
                <form onSubmit={handleSubmit} className="p-5 space-y-4">
                    <div>
                        <label className="block text-sm font-semibold text-slate-700 dark:text-[#cccccc] mb-1.5">Room Prefix</label>
                        <input
                            type="text"
                            required
                        ...
                        />
                        <p className="text-xs text-slate-500 mt-1.5">Rooms will be generated as {formData.roomPrefix}-01, {formData.roomPrefix}-02, etc.</p>
                    </div>

                    <div>
                        <label className="block text-sm font-semibold text-slate-700 dark:text-[#cccccc] mb-1.5">Room Type</label>
                        <select
                            required
                            className="w-full px-4 py-2.5 rounded-lg border border-slate-200 dark:border-[#2a2a2a] 
