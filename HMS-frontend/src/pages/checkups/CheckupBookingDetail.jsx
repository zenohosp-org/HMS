...
const RESULT_STATUS_CONFIG = {
  PENDING:        { label: "Pending",        color: "text-slate-400 dark:text-[#555]" },
  ...
  NOT_APPLICABLE: { label: "N/A",            color: "text-slate-400 dark:text-[#555]" },
};
...
function InfoRow({ icon: Icon, label, value }) {
  ...
  return (
    <div className="flex items-start gap-3">
      ...
      <div>
        <p className="text-[10px] font-bold uppercase tracking-widest text-slate-400 dark:text-[#555] mb-0.5">{label}</p>
...
function ResultRow({ result, onSave, disabled }) {
  ...
  const inputCls = "w-full px-3 py-2 rounded-lg border border-slate-200 dark:border-[#333] bg-white dark:bg-[#0e0e0e] text-slate-900 dark:text-white text-sm focus:outline-none focus:ring-2 focus:ring-emerald-500/30 focus:border-emerald-500";
      ...
      <td className="px-4 py-3 align-top">
        <div className="flex items-center gap-2">
          {result.resultStatus === "NORMAL" ? (
            ...
            <Circle className="w-3.5 h-3.5 text-slate-300 dark:text-[#444] shrink-0" />
          )}
          <div>
            <p className="text-sm font-semibold text-slate-800 dark:text-white">{result.testName}</p>
            {result.testCategory && (
              <p className="text-[10px] text-slate-400 dark:text-[#555] mt-0.5 font-medium uppercase tracking-wide">{result.testCategory}</p>
            )}
          </div>
        </div>
      </td>
        ...
        {!disabled && (
          <button
            ...
            className={`flex items-center gap-1 px-3 py-1.5 rounded-lg text-xs font-bold transition-all
              ${saved ? "bg-emerald-100 text-emerald-700 dark:bg-emerald-500/10 dark:text-emerald-400" : isDirty ? "bg-emerald-500 hover:bg-emerald-600 text-white shadow-sm" : "bg-slate-100 dark:bg-[#1e1e1e] text-slate-300 dark:text-[#444] cursor-not-allowed"}`}
          >
...
function DoctorNotesPanel({ booking, onSaved, readonly }) {
  ...
  const textCls = "w-full px-3 py-2.5 rounded-xl border border-slate-200 dark:border-[#333] bg-slate-50 dark:bg-[#1a1a1a] text-slate-900 dark:text-white text-sm focus:outline-none focus:ring-2 focus:ring-emerald-500/30 focus:border-emerald-500 resize-none placeholder:text-slate-400";
  const labelCls = "block text-xs font-bold text-slate-500 dark:text-[#888] uppercase tracking-wide mb-1.5";

  return (
    <div className="bg-white dark:bg-[#111] border border-slate-200 dark:border-[#222] rounded-2xl p-6 space-y-4 print:border-slate-300">
      ...
      <div>
        ...
        {readonly ? (
          <p className="text-sm text-slate-700 dark:text-[#ccc] whitespace-pre-wrap min-h-[60px]">{doctorNotes || <span className="text-slate-300 dark:text-[#444]">—</span>}</p>
        ) : (
      ...
      <div>
        ...
        {readonly ? (
          <p className="text-sm text-slate-700 dark:text-[#ccc] whitespace-pre-wrap min-h-[40px]">{recommendation || <span className="text-slate-300 dark:text-[#444]">—</span>}</p>
        ) : (
      ...
      {!readonly && (
        <button
          ...
          className={`flex items-center gap-2 px-5 py-2.5 rounded-xl text-sm font-bold transition-all
            ${saved ? "bg-emerald-100 text-emerald-700 dark:bg-emerald-500/10 dark:text-emerald-400" : isDirty ? "bg-emerald-500 hover:bg-emerald-600 text-white shadow-lg shadow-emerald-500/20" : "bg-slate-100 dark:bg-[#1e1e1e] text-slate-300 dark:text-[#444] cursor-not-allowed"}`}
        >
...
export default function CheckupBookingDetail() {
  ...
  if (!booking) return (
    <div className="flex flex-col items-center justify-center h-64 gap-3">
      ...
      <button onClick={() => navigate("/checkups/bookings")} className="px-4 py-2 rounded-xl border border-slate-200 dark:border-[#333] text-sm font-semibold text-slate-600 dark:text-[#aaa] hover:bg-slate-50 dark:hover:bg-[#1a1a1a] transition-colors">
        ← Back to Bookings
      </button>
    </div>
  );
    ...
    <div className="space-y-6 p-6 max-w-6xl mx-auto print:p-4">
      ...
      <div className="flex items-center justify-between flex-wrap gap-3 print:hidden">
        <button
          ...
          className="flex items-center gap-2 px-3 py-2 rounded-xl border border-slate-200 dark:border-[#333] text-sm font-semibold text-slate-600 dark:text-[#aaa] hover:bg-slate-50 dark:hover:bg-[#1a1a1a] transition-colors"
        >
        ...
        <div className="flex items-center gap-2">
          {canEdit && booking.status !== "CANCELLED" && booking.status !== "NO_SHOW" && (
            <button
              ...
              disabled={cancelling}
              className="flex items-center gap-2 px-4 py-2 rounded-xl border border-rose-200 dark:border-rose-500/30 text-rose-600 dark:text-rose-400 text-sm font-bold hover:bg-rose-50 dark:hover:bg-rose-500/10 transition-colors"
            >
          ...
          {flow && (
            <button
              ...
              disabled={advancing}
              className={`flex items-center gap-2 px-5 py-2 rounded-xl text-white text-sm font-bold shadow-lg transition-all active:scale-[0.98] ${flow.cls}`}
            >
          ...
          <button
            ...
            className="flex items-center gap-2 px-4 py-2 rounded-xl bg-slate-700 hover:bg-slate-800 dark:bg-[#1e1e1e] dark:hover:bg-[#2a2a2a] text-white text-sm font-bold transition-colors"
          >
      ...
      {/* Header card */}
      <div className="bg-white dark:bg-[#111] border border-slate-200 dark:border-[#222] rounded-2xl p-6 print:border-slate-300">
        <div className="flex items-start justify-between flex-wrap gap-4">
          <div className="flex items-start gap-4">
            ...
            <div>
              ...
              <p className="text-xs text-slate-400 dark:text-[#555] mt-0.5">
          ...
          {totalCount > 0 && (
            <div className="flex flex-col items-end gap-1">
              <p className="text-xs font-bold text-slate-400 dark:text-[#555] uppercase tracking-wide">Results Progress</p>
              <div className="flex items-center gap-2">
      ...
      {/* Test Results */}
      {totalCount > 0 && (
        <div className="bg-white dark:bg-[#111] border border-slate-200 dark:border-[#222] rounded-2xl overflow-hidden print:border-slate-300">
          <div className="flex items-center gap-2 px-6 py-4 border-b border-slate-100 dark:border-[#1e1e1e]">
            ...
            <span className="ml-auto text-xs text-slate-400 dark:text-[#555]">
              ...
              <thead>
                <tr className="border-b border-slate-100 dark:border-[#1e1e1e]">
                  {["Test Name", "Normal Range", "Result Value", "Status", "Notes", ""].map(h => (
                    <th key={h} className="px-4 py-3 text-left text-[10px] font-bold uppercase tracking-wider text-slate-400 dark:text-[#555]">{h}</th>
                  ))}
                </tr>
              </thead>
              <tbody>
