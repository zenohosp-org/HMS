...
export default function StateSelect({ value, onChange, inputClassName, labelClassName, required }: StateSelectProps) {
    ...
    return (
        <div ref={containerRef} className="relative">
            ...
            >
                <span className={value ? '' : 'text-slate-400 dark:text-[#555555]'}>
                    {value || 'Select state'}
                </span>
            ...
            {open && (
                <div className="absolute z-50 mt-1 w-full rounded-xl border border-slate-200 dark:border-[#2a2a2a] bg-white dark:bg-[#111111] shadow-lg overflow-hidden">
                    <div className="p-2 border-b border-slate-100 dark:border-[#222222] flex items-center gap-2">
                    ...
                    <ul className="max-h-52 overflow-y-auto">
                        {filtered.length === 0 && (
                            <li className="px-4 py-3 text-sm text-slate-400 text-center">No states found</li>
                        )}
                        {filtered.map(s => (
                            <li
                                key={s.id}
                            ...
                            >
                                ...
                                <span className="text-xs text-slate-400 dark:text-[#555555]">{s.stateCode}</span>
                            </li>
                        ))}
                    </ul>
                </div>
