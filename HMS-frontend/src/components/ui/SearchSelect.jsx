...
function SearchSelect({
  ...
  >{!isOpen && value ? <div className="flex-1 truncate font-medium px-3 py-2 cursor-pointer">{getDisplayValue(value)}</div> : <input
    ...
    disabled={disabled}
  />}<div className="flex items-center gap-2 shrink-0 px-3 text-slate-400">{loading && isOpen && <Loader2 className="w-4 h-4 animate-spin" />}{value && !disabled && <button onClick={handleClear} className="hover:text-red-500 transition-colors"><X className="w-4 h-4" /></button>}{!value && !loading && <Search className="w-4 h-4" />}</div></div>{isOpen && !disabled && <div className="absolute z-50 w-full mt-1 bg-white border-2 border-slate-700 shadow-[4px_4px_0px_rgba(0,0,0,0.1)] max-h-60 overflow-y-auto">{loading && results.length === 0 ? <div className="p-4 text-center text-sm text-slate-500">Searching...</div> : results.length === 0 ? <div className="p-4 text-center text-sm text-slate-500">No results found.</div> : <ul className="divide-y divide-slate-100">{results.map((item, idx) => <li
    key={idx}
