...
function Pagination({ currentPage, totalPages, totalItems, pageSize, onPageChange }) {
  ...
  return <div className="flex items-center justify-between pt-3 mt-1 border-t border-slate-100 dark:border-[#1e1e1e]"><p className="text-xs text-slate-600 dark:text-[#555555]">
  ...
  ><ChevronLeft className="w-3.5 h-3.5" /></button>{pages.map(
    (p, i) => p === "\u2026" ? <span key={`ellipsis-${i}`} className="w-7 h-7 flex items-center justify-center text-xs text-slate-600 dark:text-[#555555]">…</span> : <button
      key={p}
