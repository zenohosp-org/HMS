...
function ServiceFilters({ isOpen, onClose, onFilter, specializations }) {
  ...
  return <div className="absolute right-0 top-12 w-80 bg-white dark:bg-[#111111] border border-slate-200 dark:border-[#222222] rounded-lg shadow-2xl z-50 overflow-hidden animate-in fade-in zoom-in duration-200"><div className="p-5 space-y-6">{
    ...
    className="text-xs font-semibold text-slate-900 dark:text-white dark:text-slate-400 hover:text-slate-700 transition-colors"
  >
                            Reset
                        </button></div><div className="flex flex-wrap gap-2">{specializations.map((spec) => <button
    key={spec.id}
    onClick={() => toggleDepartment(spec.name)}
    className={`flex items-center gap-1.5 px-2.5 py-1.5 rounded-lg text-xs font-medium transition-all border ${tempFilters.departments.includes(spec.name) ? "bg-slate-100 dark:bg-slate-100 text-slate-900 dark:text-white dark:text-slate-400 border-slate-200 dark:border-slate-200/30" : "bg-slate-50 dark:bg-[#1e1e1e] text-slate-500 dark:text-[#666666] border-slate-200 dark:border-[#2a2a2a] hover:border-slate-300"}`}
  >{tempFilters.departments.includes(spec.name) && <X className="w-3 h-3" />}{spec.name}</button>)}</div></div>{
    /* Amount Section */
  }<div className="space-y-3"><h3 className="text-sm font-bold text-slate-700 dark:text-[#aaaaaa]">Amount</h3><div className="relative"><SearchableSelect
    value={tempFilters.amountRange}
    onChange={(v) => setTempFilters((prev) => ({ ...prev, amountRange: v }))}
    className="w-full pl-4 pr-10 py-2.5 bg-slate-50 dark:bg-[#0a0a0a] border border-slate-200 dark:border-[#222222] rounded-lg text-xs font-medium text-slate-600 dark:text-[#888888] appearance-none cursor-pointer focus:ring-2 focus:ring-slate-300/10 outline-none transition-all"
    options={[
  ...
  >
    ...
    className={`flex items-center gap-1.5 px-3 py-1.5 rounded-lg text-xs font-medium transition-all border ${tempFilters.statuses.includes(status) ? "bg-slate-100 dark:bg-slate-100 text-slate-900 dark:text-white dark:text-slate-400 border-slate-200 dark:border-slate-200/30" : "bg-slate-50 dark:bg-[#1e1e1e] text-slate-500 dark:text-[#666666] border-slate-200 dark:border-[#2a2a2a] hover:border-slate-300"}`}
