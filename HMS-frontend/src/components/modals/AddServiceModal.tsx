...
import { hospitalServiceApi, type HospitalService, type Specialization } from '@/utils/api'
...
export default function AddServiceModal({ isOpen, onClose, service, specializations, onSuccess }: AddServiceModalProps) {
    const { user } = useAuth()
    ...
    return (
        <div className="fixed inset-0 z-[60] flex items-center justify-center p-4">
            ...
            <div className="relative w-full max-w-md bg-white dark:bg-[#111111] border border-slate-200 dark:border-[#222222] rounded-2xl shadow-2xl overflow-hidden animate-in fade-in zoom-in duration-200">
                <div className="flex items-center justify-between px-6 py-4 border-b border-slate-100 dark:border-[#1a1a1a]">
                    <h2 className="text-xl font-bold text-slate-800 dark:text-white">
                        {service ? 'Edit Service' : 'New Service'}
                    </h2>
                    <button onClick={onClose} className="p-2 hover:bg-slate-100 dark:hover:bg-[#1e1e1e] rounded-xl text-slate-400 transition-colors">
                        <X className="w-5 h-5" />
                    </button>
                </div>
                ...
                <form onSubmit={handleSubmit} className="p-6 space-y-5">
                    <div className="space-y-1.5">
                        <label className="text-sm font-semibold text-slate-700 dark:text-[#aaaaaa] ml-1">
                    ...
                    <div className="space-y-1.5">
                        ...
                        >
                            ...
                            {specializations.filter(s => s.isActive).map(s => (
                    ...
                    <div className="flex gap-3 pt-2">
                        ...
                        >
                            {loading ? <Loader2 className="w-4 h-4 animate-spin" /> : (service ? 'Update Service' : 'Add New Service')}
                        </button>
                    </div>
                </form>
            </div>
