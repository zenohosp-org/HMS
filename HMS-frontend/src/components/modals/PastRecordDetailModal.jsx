...
export default function PastRecordDetailModal({ record, onClose }) {
  ...
  return (
    ...
    >
      <div className="bg-white dark:bg-[#0f0f0f] rounded-2xl shadow-2xl w-full max-w-4xl max-h-[92vh] border border-slate-200 dark:border-[#262626] flex flex-col overflow-hidden">
        ...
        <div className="shrink-0 border-b border-slate-100 dark:border-[#1c1c1c]">
          <div className="flex items-center justify-between px-6 py-4">
              ...
              <div className="min-w-0">
                <div className="flex items-center gap-2">
                  ...
                  {items.length > 0 && record.historyType !== "PRESCRIPTION" && (
                    <span className="px-1.5 py-0.5 rounded text-[10px] font-bold bg-emerald-50 dark:bg-emerald-500/10 text-emerald-700 dark:text-emerald-300 border border-emerald-100 dark:border-emerald-500/20">
                      + {items.length} Rx
                    </span>
                  )}
                </div>
            ...
            <button
              type="button"
              onClick={onClose}
              className="p-2 rounded-lg text-slate-400 hover:text-slate-700 hover:bg-slate-100 dark:hover:bg-[#1a1a1a] transition-colors"
              aria-label="Close"
            >
              <X className="w-5 h-5" />
            </button>
        ...
        <div className="flex-1 overflow-y-auto px-7 py-6 space-y-7">
          ...
          {chiefComplaint && (
            <Section icon={<ClipboardList className="w-3.5 h-3.5" />} title="Chief complaint" tone="blue">
              <p className="text-sm text-slate-800 dark:text-[#e5e5e5] leading-relaxed">
                {chiefComplaint}
              </p>
            </Section>
          )}
          ...
          {notes && (
            <Section icon={<FileText className="w-3.5 h-3.5" />} title="Doctor's notes">
              <p className="text-sm text-slate-700 dark:text-[#ccc] leading-relaxed whitespace-pre-wrap">
                {notes}
              </p>
            </Section>
          )}
          ...
          {items.length > 0 && (
            ...
            >
              <div className="rounded-xl border border-slate-200 dark:border-[#1c1c1c] overflow-hidden">
                <div className="grid grid-cols-12 gap-2 px-4 py-2.5 bg-slate-50 dark:bg-[#141414] border-b border-slate-100 dark:border-[#1c1c1c] text-[10px] font-bold uppercase tracking-wider text-slate-500 dark:text-[#888]">
          ...
          {!chiefComplaint && !notes && !record.instructions && items.length === 0 && !nextVisit && (
            <div className="text-center py-10">
              <p className="text-sm text-slate-400 dark:text-[#666]">
        ...
        {/* ── Footer ──────────────────────────────────────────── */}
        <div className="shrink-0 px-6 py-3.5 border-t border-slate-100 dark:border-[#1c1c1c] bg-white dark:bg-[#0f0f0f] flex items-center justify-between gap-4">
          <div className="flex items-center gap-1.5 min-w-0 text-[11px] text-slate-500 dark:text-[#888] truncate">
            <UserIcon className="w-3 h-3 shrink-0" />
            {creator ? (
              <>
                Recorded by <span className="font-semibold text-slate-700 dark:text-[#ccc]">{creator}</span>
                {creatorRole && <span className="text-slate-400 dark:text-[#666]"> · {creatorRole}</span>}
              </>
            ) : (
              <span>Recorded</span>
            )}
...
function PrescriptionRow({ drug }) {
  ...
  return (
    <div className="grid grid-cols-12 gap-2 items-center px-4 py-2.5 bg-white dark:bg-[#0f0f0f] hover:bg-slate-50/60 dark:hover:bg-[#141414] transition-colors">
      <div className="col-span-4">
        <p className="text-sm font-semibold text-slate-900 dark:text-white truncate">
          {drug.drugName || "—"}
        </p>
        {strengthLine && (
          <p className="text-[11px] text-slate-400 dark:text-[#666] mt-0.5 truncate">
            {strengthLine}
            {drug.drugGeneric && <span className="text-slate-300 dark:text-[#555]"> · {drug.drugGeneric}</span>}
          </p>
        )}
        {drug.instructions && (
          <p className="text-[11px] italic text-slate-500 dark:text-[#888] mt-0.5 line-clamp-1">
            {drug.instructions}
          </p>
        )}
      </div>
      <div className="col-span-2 text-xs text-slate-700 dark:text-[#ccc]">{drug.dose || "—"}</div>
      <div className="col-span-2 text-xs text-slate-700 dark:text-[#ccc]">{drug.frequency || "—"}</div>
      <div className="col-span-1 text-xs text-slate-500 dark:text-[#888] text-center tabular-nums">
        {drug.durationDays != null ? `${drug.durationDays}d` : "—"}
      </div>
      <div className="col-span-1 text-sm font-semibold text-slate-900 dark:text-white text-right tabular-nums">
        {drug.quantity ?? "—"}
      </div>
      <div className="col-span-2 text-xs text-slate-500 dark:text-[#888]">{drug.route || "—"}</div>
    </div>
  );
}
