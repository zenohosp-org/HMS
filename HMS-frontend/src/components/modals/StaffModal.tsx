...
export default function StaffModal({ onClose, onSave }: Props) {
    const [form, setForm] = useState<StaffFormData>({
        email: '', password: '', firstName: '', lastName: '',
        role: 'STAFF', specialization: '', department: '',
    })
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 backdrop-blur-sm">
            <div className="card w-full max-w-lg mx-4">
                <div className="p-6 border-b border-slate-200 dark:border-slate-700 flex items-center justify-between">
                    <h2 className="text-lg font-semibold text-slate-800 dark:text-slate-200">Add Staff / Doctor</h2>
                    ...
                    {form.role === 'DOCTOR' && (
                        <div className="grid grid-cols-2 gap-4">
                            <div>
                                ...
                                <input className="input" placeholder="e.g. Cardiology" value={form.specialization}
                                    onChange={e => set('specialization', e.target.value)} />
                            </div>
                            <div>
                                <label className="label">Department</label>
                                <input className="input" placeholder="e.g. ICU" value={form.department}
