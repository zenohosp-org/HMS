...
import { X, Calendar, Clock, User, FileText, Search, ChevronLeft, ChevronRight, Stethoscope, CheckCircle } from 'lucide-react'
    ...
    return (
        <div className="select-none">
            <div className="flex items-center justify-between mb-3">
                <span className="text-sm font-semibold text-slate-800 dark:text-[#e5e5e5]">{MONTHS[view.month]} {view.year}</span>
                <div className="flex items-center gap-1">
                    <button type="button" onClick={prevMonth} className="p-1 rounded hover:bg-slate-100 dark:hover:bg-[#222222] text-slate-500 dark:text-[#888888] transition-colors">
                        <ChevronLeft className="w-4 h-4" />
                    </button>
                    <button type="button" onClick={nextMonth} className="p-1 rounded hover:bg-slate-100 dark:hover:bg-[#222222] text-slate-500 dark:text-[#888888] transition-colors">
            ...
            <div className="grid grid-cols-7 mb-1">
                {DAYS.map(d => (
                    <div key={d} className="text-center text-[10px] font-bold text-slate-400 dark:text-[#555555] uppercase py-1">{d}</div>
                ))}
            </div>
                        ...
                        <button
                            ...
                            className={`text-xs py-1.5 rounded-lg font-medium transition-colors
                                ${isSelected ? 'bg-emerald-500 text-white font-bold' :
                                  isToday ? 'bg-emerald-50 dark:bg-emerald-500/10 text-emerald-600 dark:text-emerald-400 font-bold' :
                                  isPast ? 'text-slate-300 dark:text-[#444444] cursor-not-allowed' :
                                  'text-slate-700 dark:text-[#cccccc] hover:bg-slate-100 dark:hover:bg-[#222222]'}`}
                        >
                            {day}
                        </button>
                    )
...
export default function BookAppointmentModal({ isOpen, onClose, onSuccess, selectedDate }: BookAppointmentModalProps) {
    const { user } = useAuth()
    ...
    const [type, setType] = useState<'OPD' | 'FOLLOWUP' | 'EMERGENCY' | 'TELECONSULT'>('OPD')
    ...
    // Fetch doctor appointments when doctor/date changes
    useEffect(() => {
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-900/60 backdrop-blur-sm">
            <div className="bg-white dark:bg-[#111111] rounded-2xl shadow-2xl w-full max-w-5xl max-h-[92vh] flex flex-col overflow-hidden border border-slate-200 dark:border-[#1e1e1e]">
                ...
                <div className="flex items-center justify-between px-7 py-5 border-b border-slate-200 dark:border-[#1e1e1e]">
                    <div>
                        <h2 className="text-xl font-bold text-slate-900 dark:text-white">Add Appointment</h2>
                        <p className="text-sm text-slate-500 dark:text-[#888888] mt-0.5">Schedule a new appointment for a patient.</p>
                    </div>
                    <button
                ...
                <div className="flex flex-1 overflow-hidden">
                                ...
                                <div className="grid grid-cols-2 gap-2">
                                    {TYPE_OPTIONS.map(opt => (
                                        <button
                                            ...
                                            className={`text-left px-4 py-3 rounded-xl border transition-all ${type === opt.value
                                                ? 'border-emerald-500 bg-emerald-50 dark:bg-emerald-500/10'
                                                : 'border-slate-200 dark:border-[#222222] hover:border-slate-300 dark:hover:border-[#333333]'
                                            }`}
                                        >
                                            <p className={`text-sm font-semibold ${type === opt.value ? 'text-emerald-700 dark:text-emerald-400' : 'text-slate-700 dark:text-[#cccccc]'}`}>
                                                {type === opt.value && <CheckCircle className="w-3.5 h-3.5 inline mr-1.5 mb-0.5" />}
                                                {opt.label}
                                            </p>
                                            ...
                                            {timeSlots.map(slot => (
                                                <button
                                                    ...
                                                    className={`w-full flex items-center gap-3 px-4 py-3 text-sm transition-colors text-left
                                                        ${slot.isBooked ? 'text-slate-300 dark:text-[#444444] cursor-not-allowed bg-slate-50/50 dark:bg-[#111111]' :
                                                          apptTime === slot.timeStr ? 'bg-emerald-50 dark:bg-emerald-500/10 text-emerald-700 dark:text-emerald-400 font-semibold' :
                                                          'hover:bg-slate-50 dark:hover:bg-[#151515] text-slate-700 dark:text-[#cccccc]'}`}
                                                >
                    ...
                    <div className="w-80 shrink-0 flex flex-col overflow-y-auto px-6 py-6 gap-6 bg-slate-50/50 dark:bg-[#0d0d0d]">
                        ...
                        <div>
                                ...
                                <div className="border border-slate-200 dark:border-[#222222] rounded-xl overflow-hidden mb-2 bg-white dark:bg-[#111111] max-h-44 overflow-y-auto">
                                    {filteredPatients.slice(0, 8).map(p => (
                                        <button
                                            key={p.id}
                                            type="button"
                                            onClick={() => { setPatientId(String(p.id)); setPatientSearch(''); setErrors(e => ({...e, patient: ''})) }}
                                        ...
                                        >
                                            <div className="w-7 h-7 rounded-full bg-blue-50 dark:bg-blue-500/10 flex items-center justify-center text-xs font-bold text-blue-600 dark:text-blue-400 shrink-0">
                                                {p.firstName[0]}
                                            </div>
                            ...
                            {selectedPatient && (
                                <div className="flex items-center gap-3 px-3 py-2.5 bg-emerald-50 dark:bg-emerald-500/10 border border-emerald-200 dark:border-emerald-500/20 rounded-xl mb-2">
                                    <div className="w-7 h-7 rounded-full bg-emerald-100 dark:bg-emerald-500/20 flex items-center justify-center text-xs font-bold text-emerald-700 dark:text-emerald-400 shrink-0">
                                    ...
                                    <button type="button" onClick={() => setPatientId('')} className="text-emerald-500 hover:text-emerald-700 dark:hover:text-emerald-300">
                        ...
                        {/* Select Doctor */}
                        <div>
                            <h3 className="text-base font-bold text-slate-800 dark:text-white mb-1">Select Doctor</h3>
                            <p className="text-xs text-slate-500 dark:text-[#666666] mb-4">Choose a doctor for this appointment.</p>
                            ...
                            {selectedDoctor && (
                                <div className="mt-3 p-3 bg-white dark:bg-[#111111] border border-slate-200 dark:border-[#222222] rounded-xl">
                                    <div className="flex items-center gap-3">
                                        <div className="w-9 h-9 rounded-full bg-blue-50 dark:bg-blue-500/10 flex items-center justify-center text-sm font-bold text-blue-600 dark:text-blue-400 shrink-0">
                                    ...
                                    {selectedDoctor.consultationFee != null && (
                                        <div className="mt-3 pt-3 border-t border-slate-100 dark:border-[#1e1e1e] grid grid-cols-2 gap-2">
                                            <div>
                                                <p className="text-[10px] font-bold uppercase text-slate-400 dark:text-[#555555]">Fee</p>
                                                <p className="text-sm font-bold text-slate-700 dark:text-[#cccccc]">{selectedDoctor.consultationFee}</p>
                                            </div>
                                            <div>
                                                <p className="text-[10px] font-bold uppercase text-slate-400 dark:text-[#555555]">Slot</p>
                                                <p className="text-sm font-bold text-slate-700 dark:text-[#cccccc]">{selectedDoctor.slotDurationMin} min</p>
                                            </div>
                                        </div>
                                    )}
                                </div>
