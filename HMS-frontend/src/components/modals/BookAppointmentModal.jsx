    ...
    <div className="select-none">
      <div className="flex items-center justify-between mb-3">
        ...
        <div className="flex items-center gap-1">
          <button type="button" onClick={prevMonth} className="p-1 rounded hover:bg-slate-100 dark:hover:bg-[#222222] text-slate-500 dark:text-[#888888] transition-colors"><ChevronLeft className="w-4 h-4" /></button>
        ...
        {cells.map((day, i) => {
          ...
          return (
            <button type="button" key={ds} onClick={() => pick(day)} disabled={isPast}
              className={`text-xs py-1.5 rounded-lg font-medium transition-colors ${isSelected ? "bg-emerald-500 text-white font-bold" : isToday ? "bg-emerald-50 dark:bg-emerald-500/10 text-emerald-600 dark:text-emerald-400 font-bold" : isPast ? "text-slate-300 dark:text-[#444444] cursor-not-allowed" : "text-slate-700 dark:text-[#cccccc] hover:bg-slate-100 dark:hover:bg-[#222222]"}`}
            >{day}</button>
          );
        })}
      </div>
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-900/60 backdrop-blur-sm">
      <div className="bg-white dark:bg-[#111111] rounded-2xl shadow-2xl w-full max-w-5xl max-h-[92vh] flex flex-col overflow-hidden border border-slate-200 dark:border-[#1e1e1e]">
        ...
        <div className="flex items-center justify-between px-7 py-5 border-b border-slate-200 dark:border-[#1e1e1e]">
          ...
          <button onClick={() => { resetForm(); onClose(); }} className="p-2 rounded-lg text-slate-400 hover:text-slate-600 dark:hover:text-[#cccccc] hover:bg-slate-100 dark:hover:bg-[#1e1e1e] transition-colors">
            <X className="w-5 h-5" />
          </button>
        </div>
