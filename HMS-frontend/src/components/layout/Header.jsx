...
function Header({ onMenuClick }) {
  ...
  return <header className="h-14 bg-white dark:bg-[#111111] border-b border-slate-200 dark:border-[#222222]
  ...
  ><Bell className="w-4 h-4" /><span className="absolute top-1.5 right-1.5 w-1.5 h-1.5 rounded-full bg-emerald-500" /></button>{
    /* Divider */
  }<div className="w-px h-6 bg-slate-200 dark:bg-[#222222] mx-1" />{
    /* User avatar */
  }<div className="flex items-center gap-2"><div className="w-8 h-8 rounded-full bg-slate-100 border border-slate-200 dark:bg-[#2a2a2a] dark:border-[#3a3a3a]
                            flex items-center justify-center text-xs font-bold
                            text-slate-700 dark:text-[#cccccc]">{initials}</div><div className="hidden sm:block"><p className="text-xs font-semibold text-slate-700 dark:text-[#cccccc] leading-tight">{user?.firstName} {user?.lastName}</p><p className="text-xs text-slate-400 dark:text-[#555555]">{user?.roleDisplay}</p></div></div>{
    /* Logout */
  }<button
    onClick={logout}
    ...
    aria-label="Logout"
  ><LogOut className="w-4 h-4" /></button></div></header>;
}
export {
  Header as default
};
