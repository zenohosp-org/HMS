...
public interface InvoiceRepository extends JpaRepository<Invoice, UUID> {
    ...
    @Query("SELECT COALESCE(SUM(i.total), 0.0) FROM Invoice i WHERE i.hospital.id = :hid AND i.status = com.zenlocare.HMS_backend.entity.InvoiceStatus.PAID")
    double sumPaidByHospital(@Param("hid") UUID hospitalId);

    @Query("SELECT COALESCE(SUM(i.total), 0.0) FROM Invoice i WHERE i.hospital.id = :hid AND i.status NOT IN (com.zenlocare.HMS_backend.entity.InvoiceStatus.PAID, com.zenlocare.HMS_backend.entity.InvoiceStatus.CANCELLED)")
    ...
    @Query(value = "SELECT TO_CHAR(created_at, 'YYYY-MM') as month_str, " +
            ...
            "       SUM(CASE WHEN status != 'PAID' AND status != 'CANCELLED' THEN total ELSE 0 END) as unpaid " +
            "FROM invoices " +
